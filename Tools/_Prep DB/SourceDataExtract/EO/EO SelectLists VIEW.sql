...
-- *************** Service Frequency Code *****************************************************          
	select 
			...
			StateCode = '',
			Sequence = cast(0 as int) 
	from ICServiceTbl v -- select top 100 * from ICServiceTbl order by servseqnum desc
...
set nocount on;
select * from EnrichSelectLists where EnrichLabel like '%behav%'
