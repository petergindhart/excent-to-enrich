...
select 
	...
	EnrichRole = ''
from 
	excent9710a.ExcentOnlineFL.dbo.Staff t
where isnull(t.del_flag,0)=0 
and isnull(t.email,'') <> ''
and t.email not in (select de.email from excent9710a.ExcentOnlineFL.dbo.staff de where isnull(de.del_flag,0)=0 group by de.email having count(*) > 1)
