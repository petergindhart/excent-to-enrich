...
insert @gtbl 
select distinct gstudentid, iepseqnum from excent9710a.ExcentOnlineFL.dbo.SpecialEdStudentsAndIEPs
...
-- ServDesc,
	...
	from 
		@gtbl x 
		JOIN excent9710a.ExcentOnlineFL.dbo.IEPCompleteTbl i on x.IEPSeqNum = i.IEPSeqNum 
		JOIN excent9710a.ExcentOnlineFL.dbo.ICServiceTbl v on i.IEPSeqNum = v.IEPComplSeqNum and isnull(v.del_flag,0)=0 -- select top 10 * from ServiceTbl
		-- note:  at Polk there are 2 locations named "Home" with different codes - 10 and 24.
		LEFT JOIN excent9710a.ExcentOnlineFL.dbo.CodeDescLook k on case when v.LocationDesc = 'Home' then v.LocationCode else v.LocationDesc end = case when k.LookDesc = 'Home' then k.Code else k.LookDesc end
			and UsageID = 'Location'
		LEFT JOIN excent9710a.ExcentOnlineFL.dbo.Staff t on v.ProvCode = t.StaffId -- select * from ServiceTbl where ServProv is not null -- select top 100 * from ICServiceTbl where Teacher is not null
	where 
		v.Type in (
	'SpEd', 
		'RelServ', 
