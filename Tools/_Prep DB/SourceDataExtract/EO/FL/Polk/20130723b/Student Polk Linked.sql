...
/*
select HispanicLatino, AmerIndOrALNatRace, AsianRace, BlackOrAfrAmerRace, NatHIOrOthPacIslRace, WhiteRace, count(*) tot
from Student
group by HispanicLatino, AmerIndOrALNatRace, AsianRace, BlackOrAfrAmerRace, NatHIOrOthPacIslRace, WhiteRace

select Ethnic, count(*) tot
from Student
where isnull(del_flag,0)=0
and spedstat = 1
and isnull(Ethnic,'Select') <> 'Select'
group by Ethnic
order by Ethnic

select * from codedesclook where UsageID = 'Ethnicity'
...
from excent9710a.ExcentOnlineFL.dbo.SpecialEdStudentsAndIEPs x -- 2451 -- select * from SpecialEdStudentsAndIEPs -- select * from IEPTbl
JOIN excent9710a.ExcentOnlineFL.dbo.Student s on x.GStudentID = s.GStudentID
JOIN excent9710a.ExcentOnlineFL.dbo.ReportStudentSchools h on s.gstudentid = h.gstudentid
JOIN (
	...
	from (
		...
		from excent9710a.ExcentOnlineFL.dbo.StudDisability sd join
		excent9710a.ExcentOnlineFL.dbo.DisabilityLook d on sd.DisabilityID = d.DisabilityID
		...
		select 
			...
			sd.DisabilityID, 
			sd.PrimaryDiasb,
			Sequence = (select count(*)+2 from excent9710a.ExcentOnlineFL.dbo.StudDisability dc where isnull(dc.del_flag,0)=0 and dc.PrimaryDiasb = 0 and dc.GStudentID = sd.gstudentid and dc.RecNum < sd.RecNum)
		from excent9710a.ExcentOnlineFL.dbo.StudDisability sd join
		excent9710a.ExcentOnlineFL.dbo.DisabilityLook d on sd.DisabilityID = d.DisabilityID
		where isnull(sd.del_flag,0)=0
