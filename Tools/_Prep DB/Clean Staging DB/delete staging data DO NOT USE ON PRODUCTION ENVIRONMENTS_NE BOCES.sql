...
--The DELETE statement conflicted with the REFERENCE constraint "FK_ServiceDef#DefaultLocation#ServiceDefs". The conflict occurred in database "Enrich_DCB6_CO_Mountain", table "dbo.ServiceDef", column 'DefaultLocationID'.
--Msg 547, Level 16, State 0, Line 164
--The DELETE statement conflicted with the REFERENCE constraint "FK_ServiceDefDiagnosisCode#MedicaidDiagnosisCodes". The conflict occurred in database "Enrich_DCB6_CO_Mountain", table "dbo.ServiceDefDiagnosisCode", column 'ServiceDefID'.
--Msg 547, Level 16, State 0, Line 167
--The DELETE statement conflicted with the REFERENCE constraint "FK_ServiceDefProcedure#MedicaidProcedures". The conflict occurred in database "Enrich_DCB6_CO_Mountain", table "dbo.ServiceDefProcedure", column 'ServiceDefID'.
--Msg 547, Level 16, State 0, Line 170
--The DELETE statement conflicted with the REFERENCE constraint "FK_IepServiceDef_ServiceDef". The conflict occurred in database "Enrich_DCB6_CO_Mountain", table "dbo.IepServiceDef", column 'ID'.
--Msg 547, Level 16, State 0, Line 174
--The DELETE statement conflicted with the REFERENCE constraint "FK_UserProfileServiceDefPermission#ServiceDef#". The conflict occurred in database "Enrich_DCB6_CO_Mountain", table "dbo.UserProfileServiceDefPermission", column 'ServiceDefID'.

delete sdpc from ServiceDefProcedure sdpc join ServiceDef sd on sd.ID = sdpc.ServiceDefID join PrgLocation loc on loc.ID = sd.DefaultLocationID where loc.DeletedDate is not null ; print 'ServiceDefDiagnosisCode : ' + convert(varchar(10), @@rowcount)
delete sddc from ServiceDefDiagnosisCode sddc join ServiceDef sd on sd.ID = sddc.ServiceDefID join PrgLocation loc on loc.ID = sd.DefaultLocationID where loc.DeletedDate is not null ; print 'ServiceDefProcedure : ' + convert(varchar(10), @@rowcount)
delete isd from IepServiceDef isd join servicedef sd on sd.ID =isd.ID  join PrgLocation loc on loc.ID = sd.DefaultLocationID where loc.DeletedDate is not null ; print 'IEPServiceDef : ' + convert(varchar(10), @@rowcount)
delete upsdp from UserProfileServiceDefPermission upsdp join ServiceDef sd on sd.ID = upsdp.ServiceDefID join PrgLocation loc on loc.ID = sd.DefaultLocationID where loc.DeletedDate is not null ; print 'UserProfileServiceDefPermission : ' + convert(varchar(10), @@rowcount)  
delete sd from ServiceDef sd join PrgLocation loc on loc.ID = sd.DefaultLocationID where loc.DeletedDate is not null ; print 'ServiceDef : ' + convert(varchar(10), @@rowcount) 
...
--The DELETE statement conflicted with the REFERENCE constraint "FK_PrgItemOutcome#NextStatus#ItemOutcomes". The conflict occurred in database "Enrich_DCB3_CO_Boulder", table "dbo.PrgItemOutcome", column 'NextStatusID'.
delete ito from PrgItemOutcome ito join PrgStatus st on st.ID = ito.NextStatusID WHERE st.IsExit = 1 AND st.ProgramID = 'F98A8EF2-98E2-4CAC-95AF-D7D89EF7F80C' and (st.DeletedDate is not null or st.Sequence = 99); print 'PrgItemOutcome : ' + convert(varchar(10), @@rowcount) 
...
--The DELETE statement conflicted with the REFERENCE constraint "FK_Student#ResidenceSchool#". The conflict occurred in database "Enrich_DCB3_CO_Boulder", table "dbo.Student", column 'ResidenceSchoolID'.

delete h
-- select h.*
from (select Number from School where deleteddate is null group by Number having COUNT(*) > 1) n
join School h on n.Number = h.Number and h.ManuallyEntered = 1  and h.ID NOT IN (select distinct ResidenceSchoolID from student); print 'School : ' + convert(varchar(10), @@rowcount)
...
--select * from LEGACYSPED.DataConversionLogTable
...
--;with LoadColumnCTE as (
--select lc.LoadTable, (select max(convert(varchar(36), mxlc.ID)) from VC3ETL.LoadColumn mxlc where lc.LoadTable = mxlc.LoadTable and mxlc.ColumnType = 'K')
--from VC3ETL.LoadColumn lc
--where lc.ColumnType = 'K'
--)

select *
from VC3ETL.LoadColumn lc
join VC3ETL.LoadColumn


select lt.SourceTable, lt.DestTable, lt.MapTable, lc.ColumnType, lc.SourceColumn, lc.DestColumn, lt.Sequence
from VC3ETl.LoadTable lt
join VC3ETL.LoadColumn lc on lt.ID = lc.LoadTable and lc.ColumnType = 'K'
where lt.DestTable is not null
and ExtractDatabase = '29D14961-928D-4BEE-9025-238496D144C6'
and lt.Sequence < 99
-- and lt.Enabled = 1
...
-- to delete imported staff records, proceed in this order

LEGACYSPED.MAP_PersonID
	UserProfileSchool
	UserProfileOrgUnit
	UserProfile
	Person

LEGACYSPED.MAP_PrgDocumentID	
	PrgDocument

select i.DefID, count(*) tot
from LEGACYSPED.Transform_PrgDocument d
join PrgItem i on d.ItemID = i.ID
group by i.DefID
...
set nocount on;
declare @s varchar(100), @o varchar(100), @ut char(1), @mt varchar(100), @n varchar(5), @q varchar(max), @dt varchar(100), @dq varchar(max), @DestIDcolumn varchar(10); select @n = '
'
declare O cursor for 
select o.type, s.name, o.name, lt.MapTable, lt.DestTable, c.name
from sys.schemas s 
join sys.objects o on s.schema_id = o.schema_id and s.name like '%legacy%' and o.type in ('U', 'V') 
left join sys.columns c on o.object_id = c.object_id and c.name = 'DestID'
join VC3ETL.LoadTable lt on s.name+'.'+o.name = lt.SourceTable
order by s.name, case when Sequence is null then 999 else lt.Sequence end desc, o.name

open O
fetch O into @ut, @s, @o, @mt, @dt, @DestIDColumn

while @@fetch_status = 0
begin

if (@DestIDcolumn is not null)
begin
print '--	'+@s+'.'+ @o+' - '+@dt
select @dq = 'delete d from '+@s+'.'+ @o+' m join '+DestTable+' d on m.DestID = d.ID '
from VC3ETL.LoadTable 
where MapTable = @s+'.'+ @o
print @dq
end

set @q = 'if exists (select 1 from sys.schemas s join sys.objects o on s.schema_id = o.schema_id where s.name = '''+@s+''' and o.name = '''+@o+''')
drop '+case when @ut = 'V' then 'view ' else 'table ' end+ @s+'.'+ @o+@n+@n
