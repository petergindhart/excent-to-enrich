...
--The DELETE statement conflicted with the REFERENCE constraint "FK_PrgItemRelDef#InitiatingItemOutcome#ResultingItemDefs". The conflict occurred in database "Enrich_DC2_FL_Sarasota", table "dbo.PrgItemRelDef", column 'InitiatingItemOutcomeId'.
--Msg 3902, Level 16, State 1, Line 116


-- select * from PrgItemRelDef where InitiatingItemOutcomeId in (select ID From dbo.PrgItemOutcome where NextStatusID in (select ID from PrgStatus where IsExit = 1 and ProgramID = 'F98A8EF2-98E2-4CAC-95AF-D7D89EF7F80C' and (DeletedDate is not null or Sequence = 99)))


delete dbo.PrgItemRelDef where DeletedDate is not null ; print 'PrgItemRelDef : ' + convert(varchar(10), @@rowcount) 
