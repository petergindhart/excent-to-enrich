...
CREATE TABLE AURORAX.MAP_ServiceProviderTitleID
	(
	ServiceProviderCode nvarchar(150) NOT NULL,
	DestID uniqueidentifier NOT NULL
	)
GO
...
-- #############################################################################
-- Schedule
IF  EXISTS (SELECT * FROM dbo.sysobjects WHERE id = OBJECT_ID(N'AURORAX.MAP_IepServiceID') AND OBJECTPROPERTY(id, N'IsUserTable') = 1)
...
-- #############################################################################
-- Schedule
IF  EXISTS (SELECT * FROM dbo.sysobjects WHERE id = OBJECT_ID(N'AURORAX.MAP_GoalAreaID') AND OBJECTPROPERTY(id, N'IsUserTable') = 1)
	DROP TABLE AURORAX.MAP_GoalAreaID
GO

CREATE TABLE AURORAX.MAP_GoalAreaID
	(
	GoalAreaCode nvarchar(150) NOT NULL,
	DestID uniqueidentifier NOT NULL
	)
GO
ALTER TABLE AURORAX.MAP_GoalAreaID ADD CONSTRAINT
	PK_MAP_GoalAreaID PRIMARY KEY CLUSTERED
	(
	GoalAreaCode
	)
GO
