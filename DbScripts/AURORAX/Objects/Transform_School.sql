...
-- #############################################################################
-- School
IF NOT EXISTS (SELECT * FROM dbo.sysobjects WHERE id = OBJECT_ID(N'AURORAX.MAP_SchoolID') AND OBJECTPROPERTY(id, N'IsUserTable') = 1)
BEGIN
CREATE TABLE AURORAX.MAP_SchoolID
	(
	SchoolRefID varchar(150) NOT NULL,
	DestID uniqueidentifier NOT NULL
	)  

ALTER TABLE AURORAX.MAP_SchoolID ADD CONSTRAINT
...
from 
	AURORAX.School k LEFT JOIN 
	dbo.School s on 
		k.SchoolCode = s.Number AND
		s.IsLocalOrg = 1 AND
		s.ManuallyEntered = 0 LEFT JOIN 
	AURORAX.MAP_SchoolID m on k.SchoolRefID = m.SchoolRefID LEFT JOIN 
	dbo.School t on m.DestID = t.ID LEFT JOIN
	AURORAX.Transform_OrgUnit mo on k.DistrictRefID = mo.DistrictRefID
GO
...
/*
	...
	Must match on StateCode (AURORAX.School.SchoolCode = dbo.School.Number)
	School.Number should be set in the SIS import.  If the school exists without the Number, a duplicate record will be added.


6BB2A9AD-FA9B-47EF-93FA-E4A9E99E0005	1	0	AURORAX.Transform_School	School	1	AURORAX.MAP_SchoolID	SchoolRefID	NULL	0	0	1	NULL	NULL	NULL	2011-08-31 08:18:16.360	NULL	29D14961-928D-4BEE-9025-238496D144C6	1	0	0
...
update t set 
	...
	, DeleteTrans = 0
	, UpdateTrans = 1
	, DestTableFilter = 's.DestID in (select DestID from AURORAX.MAP_SchoolID)'
	, Enabled = 1
...
FROM  School d JOIN 
	AURORAX.Transform_School  s ON s.DestID=d.ID
	AND s.DestID in (select DestID from AURORAX.MAP_SchoolID)

-- INSERT AURORAX.MAP_SchoolID
SELECT SchoolRefID, NEWID()
FROM AURORAX.Transform_School s
