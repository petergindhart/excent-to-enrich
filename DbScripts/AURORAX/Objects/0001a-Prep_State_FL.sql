...
/*
	...
	AURORAX.MAP_IepDisability is populated with ETL from legacy data.  Legacy data must have the StateCode.  
...
insert AURORAX.MAP_GoalAreaDefID 
select * from @ga where GoalAreaCode not in (select GoalAreaCode from AURORAX.MAP_GoalAreaDefID)
...
IF NOT EXISTS (SELECT * FROM dbo.sysobjects WHERE id = OBJECT_ID(N'AURORAX.MAP_PostSchoolAreaDefID') AND OBJECTPROPERTY(id, N'IsUserTable') = 1)
BEGIN
CREATE TABLE AURORAX.MAP_PostSchoolAreaDefID
(
	PostSchoolAreaCode	varchar(150) NOT NULL,
	DestID uniqueidentifier NOT NULL
)

ALTER TABLE AURORAX.MAP_PostSchoolAreaDefID ADD CONSTRAINT
PK_MAP_PostSchoolAreaDefID PRIMARY KEY CLUSTERED
(
	PostSchoolAreaCode
)

create index IX_AURORAX_MAP_PostSchoolAreaDefID_DestID on AURORAX.MAP_PrgGoalID (DestID)
...
insert AURORAX.MAP_PostSchoolAreaDefID
select * from @psa where PostSchoolAreaCode not in (select PostSchoolAreaCode from AURORAX.MAP_PostSchoolAreaDefID)
END
GO
