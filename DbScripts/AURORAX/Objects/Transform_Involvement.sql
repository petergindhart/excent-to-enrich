...
AS  
 SELECT
  ...
  EndDate = case when max(iep.IEPEndDate) > getdate() then NULL else max(iep.IEPEndDate) end,   -- school end for this IEP period.  MAX so we don't have to add to group by :-)
 ...
 FROM
  ...
  StudentSchoolHistory sh on stu.DestID = sh.StudentID AND (iep.IEPStartDate < sh.EndDate AND iep.IEPEndDate > sh.StartDate)  LEFT JOIN -- will be full joins once the studentschoolhistory and studentgradelevelhistory data is inserted
  StudentGradeLevelHistory gh on stu.DestID = gh.StudentID AND (iep.IEPStartDate < gh.EndDate AND iep.IEPEndDate > gh.StartDate) LEFT JOIN
