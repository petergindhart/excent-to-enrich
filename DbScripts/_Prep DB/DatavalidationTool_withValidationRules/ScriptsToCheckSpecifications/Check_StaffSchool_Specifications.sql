...
IF EXISTS (SELECT 1 FROM sys.schemas s join sys.objects o on s.schema_id = o.schema_id where s.name = 'Datavalidation' and o.name = 'Check_StaffSchool_Specifications')
DROP PROC Datavalidation.Check_StaffSchool_Specifications
GO

CREATE PROC Datavalidation.Check_StaffSchool_Specifications
AS
BEGIN

DECLARE @sql nVARCHAR(MAX)

SET @sql = 'DELETE Datavalidation.StaffSchool'
EXEC sp_executesql @stmt = @sql

---Validated data
DECLARE @sqlvalidated VARCHAR(MAX)
SET @sqlvalidated = 
'INSERT  Datavalidation.StaffSchool  
SELECT ss.Line_No '
...
FROM Datavalidation.ValidationRules WHERE TableName = 'StaffSchool' 
...
WHILE @Count<=@MaxCount
    ...
    END
--print @sel
SET @sqlvalidated = @sqlvalidated + @sel+ ' FROM Datavalidation.StaffSchool_Local ss'
...
FROM Datavalidation.ValidationRules WHERE TableName = 'StaffSchool' AND IsRequired =1
...
FROM Datavalidation.ValidationRules WHERE TableName = 'StaffSchool' and IsFlagfield = 1
...
FROM Datavalidation.ValidationRules WHERE TableName = 'StaffSchool' AND IsFkRelation = 1
SET @Count = 1
SET @Txtfkrel = ''
...
FROM Datavalidation.ValidationRules WHERE TableName = 'StaffSchool' AND IsLookupColumn = 1
SET @Count = 1
SET @Txtlookup = ''
...
WHILE @Count<=@MaxCount
    ...
    BEGIN
    SET @Txtlookup=@Txtlookup+' AND ss.'+(SELECT columnname FROM @tbllookup WHERE ID= @Count)+' IN ( SELECT '+(SELECT lookupcolumn FROM @tbllookup WHERE ID= @Count)+' FROM Datavalidation.'+(SELECT lookuptable FROM @tbllookup WHERE ID= @Count)+')'
    END
    ...
    BEGIN
    SET @Txtlookup=@Txtlookup+' AND ss.'+(SELECT columnname FROM @tbllookup WHERE ID= @Count)+' IN ( SELECT '+(SELECT lookupcolumn FROM @tbllookup WHERE ID= @Count)+' FROM Datavalidation.'+(SELECT lookuptable FROM @tbllookup WHERE ID= @Count)+' WHERE Type = '''+ (SELECT lookuptype FROM @tbllookup WHERE ID= @Count)+''')'
    END
    ...
    BEGIN
    SET @Txtlookup=@Txtlookup+' AND (ss.'+(SELECT columnname FROM @tbllookup WHERE ID= @Count)+' IN ( SELECT '+(SELECT lookupcolumn FROM @tbllookup WHERE ID= @Count)+' FROM Datavalidation.'+(SELECT lookuptable FROM @tbllookup WHERE ID= @Count)+' OR ss.'+(SELECT columnname FROM @tbllookup WHERE ID= @Count)+' IS NULL)'
    END
    ELSE IF ((select isrequired from @tbllookup WHERE ID=@Count)= 0 and (select lookuptable from @tbllookup WHERE ID=@Count) = 'SelectLists')
    BEGIN
    SET @Txtlookup=@Txtlookup+' AND ss.'+(SELECT columnname FROM @tbllookup WHERE ID= @Count)+' IN ( SELECT '+(SELECT lookupcolumn FROM @tbllookup WHERE ID= @Count)+' FROM Datavalidation.'+(SELECT lookuptable FROM @tbllookup WHERE ID= @Count)+' WHERE Type = '''+ (SELECT lookuptype FROM @tbllookup WHERE ID= @Count)+'''OR ss.'+(SELECT columnname FROM @tbllookup WHERE ID= @Count)+' IS NULL)'
    END
    SET @Count=@Count+1
    END
...
INSERT @tbluq
SELECT ROW_NUMBER()over(order by columnname),columnname
FROM Datavalidation.ValidationRules WHERE TableName = 'StaffSchool'  AND IsUniqueField = 1
SET @Count = 1
SET @Txtunique = ''
...
WHILE @Count<=@MaxCount
    ...
    BEGIN
    SET @Txtdatatype=@Txtdatatype+' AND Datavalidation.udf_IsInteger( ss.'+(SELECT columnname FROM @tbldttype WHERE ID = @Count)+') = 1'
    END
    ELSE IF ((SELECT datatype FROM @tbldttype WHERE ID = @Count) = 'INT' and (SELECT isrequired FROM @tbldttype WHERE ID = @Count) = 0)
    BEGIN
    SET @Txtdatatype=@Txtdatatype+' AND (Datavalidation.udf_IsInteger( ss.'+(SELECT columnname FROM @tbldttype WHERE ID = @Count)+') = 1 OR ss.'+(SELECT columnname FROM @tbldttype WHERE ID = @Count)+' IS NULL)'
    END
...
DECLARE @Txtunique VARCHAR(MAX)
SET @Txtunique = ' JOIN (SELECT StaffEmail,SchoolCode FROM Datavalidation.StaffSchool_LOCAL GROUP BY StaffEmail,SchoolCode HAVING COUNT(*)=1) ucss ON (ucss.StaffEmail = ss.StaffEmail) AND (ucss.SchoolCode = ss.SchoolCode)'
...
SELECT ss.Line_No
	...
	FROM Datavalidation.StaffSchool_LOCAL ss 
		 JOIN School sc ON sc.SchoolCode = ss.SchoolCode
		 JOIN SpedStaffMember sped ON sped.StaffEmail = ss.StaffEmail
		 JOIN (SELECT StaffEmail,SchoolCode FROM Datavalidation.StaffSchool_LOCAL GROUP BY StaffEmail,SchoolCode HAVING COUNT(*)=1) ucss
...
FROM Datavalidation.StaffSchool 
--=========================================================================
...
SELECT ''StaffSchool'',''The field '+@columnname+' is required field.'',Line_No,ISNULL(StaffEmail,'''')+''|''+ISNULL(SchoolCode,'''')
...
SET @vsql = 'INSERT Datavalidation.ValidationReport (TableName,ErrorMessage,LineNumber,Line)
SELECT ''StaffSchool'',''The issue is in the datalength of the field '+@columnname+'.'',Line_No,ISNULL(StaffEmail,'''')+''|''+ISNULL(SchoolCode,'''')
FROM Datavalidation.StaffSchool_LOCAL WHERE 1 = 1'
...
SET @vsql = 'INSERT Datavalidation.ValidationReport (TableName,ErrorMessage,LineNumber,Line)
SELECT ''StaffSchool'',''Some of the '+@parentcolumn+' does not exist in '+@parenttable+' File or were not validated successfully, but it existed in '+@tablename+'.csv.'',staff.Line_No,ISNULL(staff.StaffEmail,'''')+''|''+ISNULL(staff.SchoolCode,'''')
...
SET @vsql = 'INSERT Datavalidation.ValidationReport (TableName,ErrorMessage,LineNumber,Line)
SELECT ''StaffSchool'',''The field '+@columnname+' should have one of the value in '''+LTRIM(REPLACE(@flagrecords,''',''','/'))+''''',staff.Line_No,ISNULL(staff.StaffEmail,'''')+''|''+ISNULL(staff.SchoolCode,'''')
FROM Datavalidation.StaffSchool_LOCAL staff '
...
SET @vsql = 'INSERT Datavalidation.ValidationReport (TableName,ErrorMessage,LineNumber,Line)
SELECT ''StaffSchool'',''The combination of SchoolCode and StaffEmail is repeated.'',staff.Line_No,ISNULL(staff.StaffEmail,'''')+''|''+ISNULL(staff.SchoolCode,'''')
...
--SET @query  = ' (SELECT '+@columnname+' FROM Datavalidation.StaffSchool_LOCAL GROUP BY '+@columnname+' HAVING COUNT(*)>1) ucstaff ON ucstaff.'+@columnname+' = staff.'+@columnname+' '
SET @vsql = @vsql + @query
EXEC sp_executesql @stmt=@vsql
--PRINT @vsql
END
...
SET @vsql = 'INSERT Datavalidation.ValidationReport (TableName,ErrorMessage,LineNumber,Line)
SELECT ''StaffSchool'',''The '+@columnname+' "''+ staff.'+@columnname+'+''" does not exist in '+@lookuptable+'.csv, but it existed in '+@tablename+'.csv'',staff.Line_No,ISNULL(staff.StaffEmail,'''')+''|''+ISNULL(staff.SchoolCode,'''')
FROM Datavalidation.StaffSchool_LOCAL staff  '
...
SET @query  = '  WHERE (staff.'+@columnname+' NOT IN ( SELECT '+@lookupcolumn+' FROM Datavalidation.'+@lookuptable+') AND staff.'+@columnname+' IS NOT NULL)'
SET @vsql = @vsql + @query
EXEC sp_executesql @stmt=@vsql
--PRINT @vsql
END
...
SET @vsql = 'INSERT Datavalidation.ValidationReport (TableName,ErrorMessage,LineNumber,Line)
SELECT ''StaffSchool'',''The date format issue is in '+@columnname+'.'',Line_No,ISNULL(StaffEmail,'''')+''|''+ISNULL(SchoolCode,'''')
FROM Datavalidation.StaffSchool_LOCAL WHERE 1 = 1 '
...
SET @vsql = 'INSERT Datavalidation.ValidationReport (TableName,ErrorMessage,LineNumber,Line)
SELECT ''StaffSchool'',''The field '+@columnname+' should have integer records.'',Line_No,ISNULL(StaffEmail,'''')+''|''+ISNULL(SchoolCode,'''')
FROM Datavalidation.StaffSchool_LOCAL WHERE 1 = 1 '
