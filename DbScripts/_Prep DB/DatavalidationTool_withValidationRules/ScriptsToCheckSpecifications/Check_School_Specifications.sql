...
SELECT ''School'',''The field '+@columnname+' is required field.'',Line_No,(ISNULL(SchoolCode,'''')+''|''+ISNULL(SchoolName,'''')+''|''+ISNULL(DistrictCode,'''')+''|''+ISNULL(MinutesPerWeek,'''')) as line
...
SET @vsql = 'INSERT Datavalidation.ValidationReport (TableName,ErrorMessage,LineNumber,Line)
SELECT ''School'',''The issue is in the datalength of the field '+@columnname+'.'',Line_No,(ISNULL(SchoolCode,'''')+''|''+ISNULL(SchoolName,'''')+''|''+ISNULL(DistrictCode,'''')+''|''+ISNULL(MinutesPerWeek,'''')) as line
FROM Datavalidation.School_LOCAL  WHERE 1 = 1'
...
SET @vsql = 'INSERT Datavalidation.ValidationReport (TableName,ErrorMessage,LineNumber,Line)
SELECT ''School'',''Some of the '+@parentcolumn+' does not exist in '+@parenttable+' File or were not validated successfully, but it existed in School.csv.'',tsch.Line_No,(ISNULL(tsch.SchoolCode,'''')+''|''+ISNULL(tsch.SchoolName,'''')+''|''+ISNULL(tsch.DistrictCode,'''')+''|''+ISNULL(tsch.MinutesPerWeek,'''')) as line
...
SET @vsql = 'INSERT Datavalidation.ValidationReport (TableName,ErrorMessage,LineNumber,Line)
SELECT ''School'',''The field '+@columnname+' should have integer records.'',Line_No,(ISNULL(SchoolCode,'''')+''|''+ISNULL(SchoolName,'''')+''|''+ISNULL(DistrictCode,'''')+''|''+ISNULL(MinutesPerWeek,'''')) as line FROM Datavalidation.School_LOCAL WHERE 1 = 1 '
...
SELECT Distinct 'School','The combination SchoolCode ,DistrictCode is duplicated.',tsch.Line_No,ISNULL(tsch.SCHOOLCODE,'')+'|'+ISNULL(tsch.SCHOOLNAME,'')+'|'+ISNULL(tsch.DISTRICTCODE,'')+'|'+ISNULL(tsch.MINUTESPERWEEK,'')
FROM Datavalidation.School_LOCAL tsch
JOIN (SELECT SchoolCode,DISTRICTCODE FROM Datavalidation.School_LOCAL GROUP BY SchoolCode,DISTRICTCODE HAVING COUNT(*)>1) ucsch
