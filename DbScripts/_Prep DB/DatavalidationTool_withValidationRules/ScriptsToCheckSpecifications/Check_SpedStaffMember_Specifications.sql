IF EXISTS (SELECT 1 FROM sys.schemas s join sys.objects o on s.schema_id = o.schema_id where s.name = 'Datavalidation' and o.name = 'Check_SpedStaffMember_Specifications')
DROP PROC Datavalidation.Check_SpedStaffMember_Specifications
GO

CREATE PROC Datavalidation.Check_SpedStaffMember_Specifications
AS
BEGIN

DECLARE @sql nVARCHAR(MAX)

SET @sql = 'DELETE Datavalidation.SpedStaffMember'
EXEC sp_executesql @stmt = @sql

---Validated data
DECLARE @sqlvalidated VARCHAR(MAX)
SET @sqlvalidated = 
'INSERT  Datavalidation.SpedStaffMember  
SELECT sped.Line_No '
...
FROM Datavalidation.ValidationRules WHERE TableName = 'SpedStaffMember' 
...
WHILE @Count<=@MaxCount
    ...
    END
--print @sel
SET @sqlvalidated = @sqlvalidated + @sel+ ' FROM Datavalidation.SpedStaffMember_Local sped'
...
FROM Datavalidation.ValidationRules WHERE TableName = 'SpedStaffMember' AND IsRequired =1
...
FROM Datavalidation.ValidationRules WHERE TableName = 'SpedStaffMember' and IsFlagfield = 1
...
FROM Datavalidation.ValidationRules WHERE TableName = 'SpedStaffMember' AND IsFkRelation = 1
SET @Count = 1
SET @Txtfkrel = ''
...
FROM Datavalidation.ValidationRules WHERE TableName = 'SpedStaffMember' AND IsLookupColumn = 1
SET @Count = 1
SET @Txtlookup = ''
...
WHILE @Count<=@MaxCount
    ...
    BEGIN
    SET @Txtlookup=@Txtlookup+' AND sped.'+(SELECT columnname FROM @tbllookup WHERE ID= @Count)+' IN ( SELECT '+(SELECT lookupcolumn FROM @tbllookup WHERE ID= @Count)+' FROM Datavalidation.'+(SELECT lookuptable FROM @tbllookup WHERE ID= @Count)+')'
    END
    ...
    BEGIN
    SET @Txtlookup=@Txtlookup+' AND sped.'+(SELECT columnname FROM @tbllookup WHERE ID= @Count)+' IN ( SELECT '+(SELECT lookupcolumn FROM @tbllookup WHERE ID= @Count)+' FROM Datavalidation.'+(SELECT lookuptable FROM @tbllookup WHERE ID= @Count)+' WHERE Type = '''+ (SELECT lookuptype FROM @tbllookup WHERE ID= @Count)+''')'
    END
    ...
    BEGIN
    SET @Txtlookup=@Txtlookup+' AND (sped.'+(SELECT columnname FROM @tbllookup WHERE ID= @Count)+' IN ( SELECT '+(SELECT lookupcolumn FROM @tbllookup WHERE ID= @Count)+' FROM Datavalidation.'+(SELECT lookuptable FROM @tbllookup WHERE ID= @Count)+' OR sped.'+(SELECT columnname FROM @tbllookup WHERE ID= @Count)+' IS NULL)'
    END
    ELSE IF ((select isrequired from @tbllookup WHERE ID=@Count)= 0 and (select lookuptable from @tbllookup WHERE ID=@Count) = 'SelectLists')
    BEGIN
    SET @Txtlookup=@Txtlookup+' AND sped.'+(SELECT columnname FROM @tbllookup WHERE ID= @Count)+' IN ( SELECT '+(SELECT lookupcolumn FROM @tbllookup WHERE ID= @Count)+' FROM Datavalidation.'+(SELECT lookuptable FROM @tbllookup WHERE ID= @Count)+' WHERE Type = '''+ (SELECT lookuptype FROM @tbllookup WHERE ID= @Count)+'''OR sped.'+(SELECT columnname FROM @tbllookup WHERE ID= @Count)+' IS NULL)'
    END
    SET @Count=@Count+1
    END
...
WHILE @Count<=@MaxCount
    ...
    BEGIN
    SET @Txtdatatype=@Txtdatatype+' AND Datavalidation.udf_IsInteger( sped.'+(SELECT columnname FROM @tbldttype WHERE ID = @Count)+') = 1'
    END
    ELSE IF ((SELECT datatype FROM @tbldttype WHERE ID = @Count) = 'INT' and (SELECT isrequired FROM @tbldttype WHERE ID = @Count) = 0)
    BEGIN
    SET @Txtdatatype=@Txtdatatype+' AND (Datavalidation.udf_IsInteger( sped.'+(SELECT columnname FROM @tbldttype WHERE ID = @Count)+') = 1 OR sped.'+(SELECT columnname FROM @tbldttype WHERE ID = @Count)+' IS NULL)'
    END
...
INSERT @tbluq
SELECT ROW_NUMBER()over(order by columnname),columnname
FROM Datavalidation.ValidationRules WHERE TableName = 'SpedStaffMember'  AND IsUniqueField = 1
SET @Count = 1
SET @Txtunique = ''
SET @MaxCount = (SELECT MAX(id)FROM @tbluq)
WHILE @Count<=@MaxCount
    BEGIN
        SET @Txtunique=@Txtunique + ' JOIN (SELECT ' +(select columnname from @tbluq WHERE ID=@Count)+ ' FROM Datavalidation.SpedStaffMember_LOCAL GROUP BY '+(select columnname from @tbluq WHERE ID=@Count)+' HAVING COUNT(*)=1) ucsped ON ucsped.' +(select columnname from @tbluq WHERE ID=@Count) +'= sped. '+(select columnname from @tbluq WHERE ID=@Count)
...
SELECT spedstaff.Line_No
	...
	FROM Datavalidation.SpedStaffMember_LOCAL spedstaff 
		 JOIN (SELECT StaffEmail FROM Datavalidation.SpedStaffMember_LOCAL GROUP BY StaffEmail HAVING COUNT(*)=1) ucspedstaff ON spedstaff.STAFFEMAIL = ucspedstaff.STAFFEMAIL
    WHERE ((DATALENGTH(spedstaff.STAFFEMAIL)/2)<= 150 OR spedstaff.STAFFEMAIL IS NULL) 
		  ...
		  --AND NOT EXISTS (SELECT StaffEmail FROM Datavalidation.SpedStaffMember_LOCAL GROUP BY StaffEmail HAVING COUNT(*)>1) 
...
SELECT ''SpedStaffMember'',''The field '+@columnname+' is required field.'',Line_No,ISNULL(StaffEmail,'''')+''|''+ISNULL(LASTNAME,'''')+ISNULL(Firstname,'''')+''|''+ISNULL(EnrichRole,'''')
FROM Datavalidation.SpedStaffMember_LOCAL WHERE 1 = 1'
...
SET @vsql = 'INSERT Datavalidation.ValidationReport (TableName,ErrorMessage,LineNumber,Line)
SELECT ''SpedStaffMember'',''The issue is in the datalength of the field '+@columnname+'.'',Line_No,ISNULL(StaffEmail,'''')+''|''+ISNULL(LASTNAME,'''')+ISNULL(Firstname,'''')+''|''+ISNULL(EnrichRole,'''')
FROM Datavalidation.SpedStaffMember_LOCAL WHERE 1 = 1'
...
SET @vsql = 'INSERT Datavalidation.ValidationReport (TableName,ErrorMessage,LineNumber,Line)
SELECT ''SpedStaffMember'',''Some of the '+@parentcolumn+' does not exist in '+@parenttable+' File or were not validated successfully, but it existed in SpedStaffMember.csv.'',Line_No,ISNULL(sped.StaffEmail,'''')+''|''+ISNULL(sped.LASTNAME,'''')+ISNULL(sped.Firstname,'''')+''|''+ISNULL(sped.EnrichRole,'''')
FROM Datavalidation.SpedStaffMember_LOCAL sped'

SET @query  = ' LEFT JOIN Datavalidation.'+@parenttable+' dt ON sped.'+@columnname+' = dt.'+@parentcolumn+' WHERE dt.'+@parentcolumn+' IS NULL'
SET @vsql = @vsql + @query
EXEC sp_executesql @stmt=@vsql
--PRINT @vsql
END
...
SET @vsql = 'INSERT Datavalidation.ValidationReport (TableName,ErrorMessage,LineNumber,Line)
SELECT ''SpedStaffMember'',''The field '+@columnname+' should have one of the value in '''+LTRIM(REPLACE(@flagrecords,''',''','/'))+''''',Line_No,ISNULL(StaffEmail,'''')+''|''+ISNULL(LASTNAME,'''')+ISNULL(Firstname,'''')+''|''+ISNULL(EnrichRole,'''')
...
SET @vsql = 'INSERT Datavalidation.ValidationReport (TableName,ErrorMessage,LineNumber,Line)
SELECT ''SpedStaffMember'',''The field '+@columnname+' is unique field, Here '+@columnname+' record is repeated.'',sped.Line_No,ISNULL(sped.StaffEmail,'''')+''|''+ISNULL(sped.LASTNAME,'''')+ISNULL(sped.Firstname,'''')+''|''+ISNULL(sped.EnrichRole,'''')
FROM Datavalidation.SpedStaffMember_LOCAL sped JOIN'

SET @query  = ' (SELECT '+@columnname+' FROM Datavalidation.SpedStaffMember_LOCAL GROUP BY '+@columnname+' HAVING COUNT(*)>1) ucsped ON ucsped.'+@columnname+' = sped.'+@columnname+' '
SET @vsql = @vsql + @query
EXEC sp_executesql @stmt=@vsql
--PRINT @vsql
END
...
SET @vsql = 'INSERT Datavalidation.ValidationReport (TableName,ErrorMessage,LineNumber,Line)
SELECT ''SpedStaffMember'',''The '+@columnname+' "''+ sped.'+@columnname+'+''" does not exist in '+@lookuptable+'.csv, but it existed in '+@tablename+'.csv'',sped.Line_No,ISNULL(sped.StaffEmail,'''')+''|''+ISNULL(sped.LASTNAME,'''')+ISNULL(sped.Firstname,'''')+''|''+ISNULL(sped.EnrichRole,'''')
FROM Datavalidation.SpedStaffMember_LOCAL sped '

SET @query  = ' WHERE (sped.'+@columnname+' NOT IN ( SELECT '+@lookupcolumn+' FROM Datavalidation.'+@lookuptable+' WHERE Type = '''+@lookuptype+''') AND sped.'+@columnname+' IS NOT NULL)'
SET @vsql = @vsql + @query
EXEC sp_executesql @stmt=@vsql
--PRINT @vsql
END
...
SET @query  = ' WHERE (sped.'+@columnname+' NOT IN ( SELECT '+@lookupcolumn+' FROM Datavalidation.'+@lookuptable+') AND sped.'+@columnname+' IS NOT NULL)'
SET @vsql = @vsql + @query
EXEC sp_executesql @stmt=@vsql
--PRINT @vsql
END
...
SET @vsql = 'INSERT Datavalidation.ValidationReport (TableName,ErrorMessage,LineNumber,Line)
SELECT ''SpedStaffMember'',''The field '+@columnname+' should have integer records.'',Line_No,ISNULL(StaffEmail,'''')+''|''+ISNULL(LASTNAME,'''')+ISNULL(Firstname,'''')+''|''+ISNULL(EnrichRole,'''')
FROM Datavalidation.SpedStaffMember_LOCAL WHERE 1 = 1 '
