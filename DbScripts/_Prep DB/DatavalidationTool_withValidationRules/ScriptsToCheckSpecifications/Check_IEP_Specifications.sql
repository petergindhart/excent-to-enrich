...
WHILE @Count<=@MaxCount
    BEGIN
    SET @Txtfkrel=@Txtfkrel+' JOIN Datavalidation.'+(SELECT parenttable FROM @tblfkrel WHERE ID= @Count)+' '+(SELECT LEFT(parenttable,3) FROM @tblfkrel WHERE ID= @Count)+' ON '+(SELECT LEFT(parenttable,3) FROM @tblfkrel WHERE ID= @Count)+'.'+(SELECT parentcolumn FROM @tblfkrel WHERE ID= @Count)+' = i.'+(SELECT columnname FROM @tblfkrel WHERE ID= @Count)
    SET @Count=@Count+1
    END
...
SET @vsql = 'INSERT Datavalidation.ValidationReport (TableName,ErrorMessage,LineNumber,Line)
SELECT ''IEP'',''The issue is in the datalength of the field '+@columnname+'.'',Line_No,ISNULL(IepRefID,'''')+''|''+ISNULL(StudentRefID,'''')+''|''+ISNULL(IEPMeetDate,'''')+''|''+ISNULL(IEPStartDate,'''')+ISNULL(IEPEndDate,'''')+''|''+ISNULL(NextReviewDate,'''')+''|''+ISNULL(InitialEvaluationDate,'''')+''|''+ISNULL(LatestEvaluationDate,'''')+''|''+ISNULL(NextEvaluationDate,'''')+ISNULL(EligibilityDate,'''')+''|''+ISNULL(ConsentForServicesDate,'''')+''|''+ISNULL(ConsentForEvaluationDate,'''')+''|''+ISNULL(LREAgeGroup,'''')+''|''+ISNULL(LRECode,'''')+''|''+ISNULL(MinutesPerWeek,'''')+''|''+ISNULL(ServiceDeliveryStatement,'''')
FROM Datavalidation.IEP_LOCAL  WHERE 1 = 1'
...
SET @vsql = 'INSERT Datavalidation.ValidationReport (TableName,ErrorMessage,LineNumber,Line)
SELECT ''IEP'',''Some of the '+@parentcolumn+' does not exist in '+@parenttable+' File or were not validated successfully, but it existed in IEP.csv.'',iep.Line_No,ISNULL(iep.IepRefID,'''')+''|''+ISNULL(iep.StudentRefID,'''')+''|''+ISNULL(iep.IEPMeetDate,'''')+''|''+ISNULL(iep.IEPStartDate,'''')+ISNULL(iep.IEPEndDate,'''')+''|''+ISNULL(iep.NextReviewDate,'''')+''|''+ISNULL(iep.InitialEvaluationDate,'''')+''|''+ISNULL(iep.LatestEvaluationDate,'''')+''|''+ISNULL(iep.NextEvaluationDate,'''')+ISNULL(iep.EligibilityDate,'''')+''|''+ISNULL(iep.ConsentForServicesDate,'''')+''|''+ISNULL(iep.ConsentForEvaluationDate,'''')+''|''+ISNULL(iep.LREAgeGroup,'''')+''|''+ISNULL(iep.LRECode,'''')+''|''+ISNULL(iep.MinutesPerWeek,'''')+''|''+ISNULL(iep.ServiceDeliveryStatement,'''')
FROM Datavalidation.IEP_LOCAL iep'
...
SET @vsql = 'INSERT Datavalidation.ValidationReport (TableName,ErrorMessage,LineNumber,Line)
SELECT ''IEP'',''The field '+@columnname+' should have one of the value in '''+LTRIM(REPLACE(@flagrecords,''',''','/'))+''''',Line_No,ISNULL(IepRefID,'''')+''|''+ISNULL(StudentRefID,'''')+''|''+ISNULL(IEPMeetDate,'''')+''|''+ISNULL(IEPStartDate,'''')+ISNULL(IEPEndDate,'''')+''|''+ISNULL(NextReviewDate,'''')+''|''+ISNULL(InitialEvaluationDate,'''')+''|''+ISNULL(LatestEvaluationDate,'''')+''|''+ISNULL(NextEvaluationDate,'''')+ISNULL(EligibilityDate,'''')+''|''+ISNULL(ConsentForServicesDate,'''')+''|''+ISNULL(ConsentForEvaluationDate,'''')+''|''+ISNULL(LREAgeGroup,'''')+''|''+ISNULL(LRECode,'''')+''|''+ISNULL(MinutesPerWeek,'''')+''|''+ISNULL(ServiceDeliveryStatement,'''')
...
SET @vsql = 'INSERT Datavalidation.ValidationReport (TableName,ErrorMessage,LineNumber,Line)
SELECT ''IEP'',''The field '+@columnname+' is unique field, Here '+@columnname+' record is repeated.'',iep.Line_No,ISNULL(iep.IepRefID,'''')+''|''+ISNULL(iep.StudentRefID,'''')+''|''+ISNULL(iep.IEPMeetDate,'''')+''|''+ISNULL(iep.IEPStartDate,'''')+ISNULL(iep.IEPEndDate,'''')+''|''+ISNULL(iep.NextReviewDate,'''')+''|''+ISNULL(iep.InitialEvaluationDate,'''')+''|''+ISNULL(iep.LatestEvaluationDate,'''')+''|''+ISNULL(iep.NextEvaluationDate,'''')+ISNULL(iep.EligibilityDate,'''')+''|''+ISNULL(iep.ConsentForServicesDate,'''')+''|''+ISNULL(iep.ConsentForEvaluationDate,'''')+''|''+ISNULL(iep.LREAgeGroup,'''')+''|''+ISNULL(iep.LRECode,'''')+''|''+ISNULL(iep.MinutesPerWeek,'''')+''|''+ISNULL(iep.ServiceDeliveryStatement,'''')
...
SET @vsql = 'INSERT Datavalidation.ValidationReport (TableName,ErrorMessage,LineNumber,Line)
SELECT ''IEP'',''The '+@columnname+' "''+ iep.'+@columnname+'+''" does not exist in '+@lookuptable+'.csv, but it existed in '+@tablename+'.csv'',iep.Line_No,ISNULL(iep.IepRefID,'''')+''|''+ISNULL(iep.StudentRefID,'''')+''|''+ISNULL(iep.IEPMeetDate,'''')+''|''+ISNULL(iep.IEPStartDate,'''')+ISNULL(iep.IEPEndDate,'''')+''|''+ISNULL(iep.NextReviewDate,'''')+''|''+ISNULL(iep.InitialEvaluationDate,'''')+''|''+ISNULL(iep.LatestEvaluationDate,'''')+''|''+ISNULL(iep.NextEvaluationDate,'''')+ISNULL(iep.EligibilityDate,'''')+''|''+ISNULL(iep.ConsentForServicesDate,'''')+''|''+ISNULL(iep.ConsentForEvaluationDate,'''')+''|''+ISNULL(iep.LREAgeGroup,'''')+''|''+ISNULL(iep.LRECode,'''')+''|''+ISNULL(iep.MinutesPerWeek,'''')+''|''+ISNULL(iep.ServiceDeliveryStatement,'''')
FROM Datavalidation.IEP_LOCAL iep '
...
BEGIN
SET @vsql = 'INSERT Datavalidation.ValidationReport (TableName,ErrorMessage,LineNumber,Line)
SELECT ''IEP'',''The date format issue is in '+@columnname+'.'',Line_No,ISNULL(IepRefID,'''')+''|''+ISNULL(StudentRefID,'''')+''|''+ISNULL(IEPMeetDate,'''')+''|''+ISNULL(IEPStartDate,'''')+ISNULL(IEPEndDate,'''')+''|''+ISNULL(NextReviewDate,'''')+''|''+ISNULL(InitialEvaluationDate,'''')+''|''+ISNULL(LatestEvaluationDate,'''')+''|''+ISNULL(NextEvaluationDate,'''')+ISNULL(EligibilityDate,'''')+''|''+ISNULL(ConsentForServicesDate,'''')+''|''+ISNULL(ConsentForEvaluationDate,'''')+''|''+ISNULL(LREAgeGroup,'''')+''|''+ISNULL(LRECode,'''')+''|''+ISNULL(MinutesPerWeek,'''')+''|''+ISNULL(ServiceDeliveryStatement,'''')
FROM Datavalidation.IEP_LOCAL WHERE 1 = 1 '
...
SET @vsql = 'INSERT Datavalidation.ValidationReport (TableName,ErrorMessage,LineNumber,Line)
SELECT ''IEP'',''The field '+@columnname+' should have integer records.'',Line_No,ISNULL(IepRefID,'''')+''|''+ISNULL(StudentRefID,'''')+''|''+ISNULL(IEPMeetDate,'''')+''|''+ISNULL(IEPStartDate,'''')+ISNULL(IEPEndDate,'''')+''|''+ISNULL(NextReviewDate,'''')+''|''+ISNULL(InitialEvaluationDate,'''')+''|''+ISNULL(LatestEvaluationDate,'''')+''|''+ISNULL(NextEvaluationDate,'''')+ISNULL(EligibilityDate,'''')+''|''+ISNULL(ConsentForServicesDate,'''')+''|''+ISNULL(ConsentForEvaluationDate,'''')+''|''+ISNULL(LREAgeGroup,'''')+''|''+ISNULL(LRECode,'''')+''|''+ISNULL(MinutesPerWeek,'''')+''|''+ISNULL(ServiceDeliveryStatement,'''')  FROM Datavalidation.IEP_LOCAL WHERE 1 = 1 '
