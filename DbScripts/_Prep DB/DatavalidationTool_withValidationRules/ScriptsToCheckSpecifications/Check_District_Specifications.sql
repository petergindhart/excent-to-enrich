...
WHILE @Count<=@MaxCount
    BEGIN
        SET @Txtunique=@Txtunique + ' JOIN  (SELECT ' +(select columnname from @tbluq WHERE ID=@Count)+ ' FROM Datavalidation.District_LOCAL GROUP BY '+(select columnname from @tbluq WHERE ID=@Count)+' HAVING COUNT(*)=1) ucdt ON ucdt.' +(select columnname from @tbluq WHERE ID=@Count) +'= dt. '+(select columnname from @tbluq WHERE ID=@Count)
...
SELECT ''District'',''The field '+@columnname+' is required field.'',Line_No,(ISNULL(DISTRICTCODE,'''')+''|''+ISNULL(DISTRICTNAME,'''')) as line
...
SET @vsql = 'INSERT Datavalidation.ValidationReport (TableName,ErrorMessage,LineNumber,Line)
SELECT ''District'',''The issue is in the datalength of the field '+@columnname+'.'',Line_No,(ISNULL(DISTRICTCODE,'''')+''|''+ISNULL(DISTRICTNAME,'''')) as line
FROM Datavalidation.District_Local WHERE 1 = 1'
...
SET @vsql = 'INSERT Datavalidation.ValidationReport (TableName,ErrorMessage,LineNumber,Line)
SELECT ''District'',''The field '+@columnname+' is unique field'',dt.Line_No,(ISNULL(dt.DISTRICTCODE,'''')+''|''+ISNULL(dt.DISTRICTNAME,'''')) as line
