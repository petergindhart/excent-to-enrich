...
and a.Placement is not null -- 11404  
...
select * 
from ICAssessmentTbl a 


  
select s.StudentIndex
from Student s
join IEPTbl i on s.GStudentID = i.GStudentID
where IEPComplete = 'IEPComplete'
and s.SpedStat = 1
...
where s.enrollstat = 1 -- 2010
and s.SpedStat = 1 -- 2004
and s.GStudentID in (select sd.GStudentID from StudDisability sd join DisabilityLook d on sd.DisabilityID = d.DisabilityID where isnull(sd.del_flag,0)=0 and sd.PrimaryDiasb = 1)  -- 2000
and ic.CurEvalDate is not null -- 996
and ic.InitDate is not null -- 969
...
select * from SpecialEdStudentsAndIEPs
