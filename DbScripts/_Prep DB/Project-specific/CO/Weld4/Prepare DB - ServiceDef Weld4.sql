...
------ delete test		-- Delete non-Enrich servicedef from Weld4 database before conversion
select x.*
...
delete x
-- select x.*
from UserProfileServiceDefPermission x left join
@ServiceDef d on x.ID = d.ID 
where d.ID is null
...
delete x
-- select x.*
from IepServiceDef x left join
@ServiceDef d on x.ID = d.ID 
where d.ID is null
...
-- select * from ServiceDef order by UserDefined
