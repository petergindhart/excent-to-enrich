...
-- The output of this view will be the one GoalArea selected for each Goal.
	...
	from LEGACYSPED.Goal g JOIN
		...
		LEGACYSPED.MAP_IepGoalAreaDefID m on g.GoalAreaCode = m.GoalAreaCode -- select * from LEGACYSPED.MAP_GoalAreaDefID -- select * from IepGoalAreaDef order by deleteddate, sequence, Name -- select * from LEGACYSPED.MAP_GoalAreaDefID
	--where ga.GoalIndex = (				
...
WHERE NOT EXISTS (SELECT * FROM IepGoalArea d WHERE s.DestID=d.ID) -- 4274
...
SELECT InstanceID, DefID, count(*) tot
FROM LEGACYSPED.Transform_IepGoalArea s
WHERE NOT EXISTS (SELECT * FROM IepGoalArea d WHERE s.DestID=d.ID) -- 4274
group by InstanceID, DefID
having count(*) > 1
order by instanceid, defid





D9902B4D-DDCF-47BD-9285-08AADFB57738	D9260B1A-F268-4304-A22B-34A9D8001DE0	4

select * from LEGACYSPED.Transform_IepGoalArea where Instanceid = 'D9902B4D-DDCF-47BD-9285-08AADFB57738' 
...
from LEGACYSPED.Goal g JOIN
	LEGACYSPED.Transform_PrgGoals pgs on g.IepRefID = pgs.IepRefID join -- on left join some records do not have an instanceid  -- 4E367F51-09E0-41A6-9CA1-88F0230A05D1 
	LEGACYSPED.MAP_GoalAreaDefID m on g.GoalAreaCode = m.GoalAreaCode 
where
g.ieprefid = 'AC54A0CC-5AE7-4699-ADBF-B7B6830DC966'


select * from LEGACYSPED.MAP_GoalAreaDefID 
