...
-- NOTE:  DO NOT TOUCH THE RECORDS ADDED BY SIS IMPORT OR MANUALLY ENTERED STUDENTS.  SIS RECORDS DO NEED TO BE MAPPED.  NEW RECORDS FROM SPED NEED TO BE ADDED.
 ...
 FROM
  ...
  LEGACYSPED.Transform_School sch on (src.ServiceSchoolCode = sch.SchoolCode and src.ServiceDistrictCode = sch.DistrictCode) OR (src.HomeSchoolCode = sch.SchoolCode and src.ServiceDistrictCode = sch.DistrictCode) LEFT JOIN
  dbo.Student s on src.StudentStateID = s.Number and /* and s.IsActive = 1 -- removed 20111114 because this was adding a duplicate student.  We will leave them inactive, though */ 
	s.ID = (
		select top 1 a.ID 
		from dbo.Student a 
		where
  ...
  PrgItem i on coalesce(s.ID, m.DestID) = i.StudentID and i.DefID = '8011D6A2-1014-454B-B83C-161CE678E3D3' left join 
	( 
