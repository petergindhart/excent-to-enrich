...
IF  EXISTS (SELECT 1 FROM sys.objects WHERE object_id = OBJECT_ID(N'LEGACYSPED.Lookups_LOCAL') AND type in (N'U'))
DROP TABLE LEGACYSPED.Lookups_LOCAL
GO

CREATE TABLE LEGACYSPED.Lookups_LOCAL(
Type	varchar(20),
SubType	varchar(20),
...
IF  EXISTS (SELECT 1 FROM sys.views WHERE object_id = OBJECT_ID(N'LEGACYSPED.Lookups'))
DROP VIEW LEGACYSPED.Lookups
GO


CREATE VIEW LEGACYSPED.Lookups
AS
	SELECT * FROM LEGACYSPED.Lookups_LOCAL
GO


IF  EXISTS (SELECT 1 FROM sys.views WHERE object_id = OBJECT_ID(N'LEGACYSPED.LookupsUniqueLabel'))
DROP VIEW LEGACYSPED.LookupsUniqueLabel
GO

CREATE VIEW LEGACYSPED.LookupsUniqueLabel
as
--/*
...
--	from LEGACYSPED.LookupsUniqueLabel k
--	where Type = 'Service'
--	union
--	select SubType, Code, Label
--	from LEGACYSPED.LookupsPreferences p 
--	where Type = 'Service'
...
from LEGACYSPED.Lookups k left join
	(
