...
-- IepGoal
 ...
 FROM
  LEGACYSPED.Goal g LEFT JOIN -- 34737
  LEGACYSPED.MAP_PrgGoalID m on g.GoalRefID = m.GoalRefID LEFT JOIN -- 34737
...
declare @t uniqueidentifier ; select @t = id from VC3ETL.LoadTable where ExtractDatabase = '29D14961-928D-4BEE-9025-238496D144C6' and DestTable = @n And SourceTable = 'LEGACYSPED.Transform_PrgGoal'
-- select * from vc3etl.loadtable where id = @t
update t set Enabled = 1
	, HasMapTable = 1
	, MapTable = 'LEGACYSPED.MAP_PrgGoalID'
	, KeyField = 'GoalRefID'
	, DeleteKey = 'DestID'
	, DeleteTrans = 1
	, UpdateTrans = 1
...
select d.*
-- DELETE LEGACYSPED.MAP_PrgGoalID
FROM LEGACYSPED.Transform_PrgGoal AS s RIGHT OUTER JOIN 
	LEGACYSPED.MAP_PrgGoalID as d ON s.DestID = d.DestID
...
FROM  PrgGoal d JOIN 
	LEGACYSPED.Transform_PrgGoal  s ON s.DestID=d.ID

-- INSERT LEGACYSPED.MAP_PrgGoalID
SELECT GoalRefID, NEWID()
FROM LEGACYSPED.Transform_PrgGoal s
