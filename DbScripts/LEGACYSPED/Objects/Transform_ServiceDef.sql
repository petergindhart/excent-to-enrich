...
select 
	...
	ExcludesID = t.ExcludesID, -- Imported at individual service level, not here
	...
	MedicaidServiceID = '2918EA73-313C-4470-AA14-CE4D91866EDC' -- Educational Services (?)
from LEGACYSPED.SelectLists k left join 
	...
	) n on n.ServiceDefName = k.EnrichLabel and isnull(n.ServiceCategoryName,'') = case isnull(k.SubType,'')
		when 'SpecialEd' then 'ServiceDef'
		when 'Related' then 'Related Services'
		when 'Supplemental' then 'Supplementary Aids and Services'
		else isnull(k.SubType,'')
		end left join 
	dbo.ServiceDef ssd on ssd.StateCode = k.StateCode left join
