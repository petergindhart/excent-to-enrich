...
AS
	...
	FROM dbo.IepPlacementType t CROSS JOIN
		...
		LEGACYSPED.Transform_IepPlacementOption p on 
			...
			iep.LRECode = p.PlacementOptionCode LEFT JOIN -- select * from LEGACYSPED.Transform_IepPlacementOption -- go back and return all rows in the transform, making sure that the map populating queries aren't affected.
		LEGACYSPED.MAP_IepPlacementID m on m.IepRefID = iep.IepRefID and t.ID = m.TypeID LEFT JOIN -- select * from LEGACYSPED.MAP_IEPPlacement
...
update t set 
	...
	, HasMapTable = 1
	, MapTable = 'LEGACYSPED.MAP_IepPlacementID'
...
DELETE LEGACYSPED.MAP_IepPlacementID
FROM LEGACYSPED.Transform_IepPlacement AS s RIGHT OUTER JOIN 
	LEGACYSPED.MAP_IepPlacementID as d ON s.DestID = d.DestID
...
FROM  IepPlacement d JOIN 
	LEGACYSPED.Transform_IepPlacement  s ON s.DestID=d.ID

INSERT LEGACYSPED.MAP_IepPlacementID
SELECT IepRefID, TypeId, NEWID()
FROM LEGACYSPED.Transform_IepPlacement s
WHERE NOT EXISTS (SELECT * FROM IepPlacement d WHERE s.DestID=d.ID)

Msg 2627, Level 14, State 1, Line 12
Violation of PRIMARY KEY constraint 'PK_MAP_IepPlacement'. Cannot insert duplicate key in object 'LEGACYSPED.MAP_IepPlacementID'.
...
select * from LEGACYSPED.MAP_IepPlacementID where IepRefID = '0F122B76-0855-4448-874A-8BBAC4C26CA2'
select * from LEGACYSPED.IEP where IepRefID = '0F122B76-0855-4448-874A-8BBAC4C26CA2'
select * from LEGACYSPED.Transform_IepPlacement where IepRefID = '0F122B76-0855-4448-874A-8BBAC4C26CA2'
select * from LEGACYSPED.Transform_IepLeastRestrictiveEnvironment where IepRefID = '0F122B76-0855-4448-874A-8BBAC4C26CA2'

select m.*
from LEGACYSPED.MAP_IepPlacementID m left join 
...
FROM 
	...
	LEGACYSPED.Transform_Iep iep JOIN
	LEGACYSPED.Student s on iep.StudentRefID = s.StudentRefID JOIN
	LEGACYSPED.Transform_LRE lre ON iep.IepRefID = lre.IepRefId LEFT JOIN 
	LEGACYSPED.Transform_IepPlacementOptionID p on iep.AgeGroup+iep.LRECode = p.SubType+p.LRECode LEFT JOIN -- select * from LEGACYSPED.Transform_IepPlacementOptionID
	LEGACYSPED.MAP_IepPlacement m on m.IepRefID = iep.IepRefID and p.TypeID = m.TypeID and t.TypeID = m.TypeID LEFT JOIN -- select * from LEGACYSPED.MAP_IEPPlacement
	dbo.IepPlacement place on lre.DestID = place.ID 
...
select * from LEGACYSPED.Transform_IepPlacementOptionID
...
select 
	...
	FROM 
		...
		LEGACYSPED.Student s on iep.StudentRefID = s.StudentRefID JOIN
		LEGACYSPED.Transform_LRE lre ON iep.IepRefID = lre.IepRefId LEFT JOIN 
		LEGACYSPED.Transform_IepPlacementOptionID p on iep.AgeGroup+iep.LRECode = p.SubType+p.LRECode LEFT JOIN
		LEGACYSPED.MAP_IepPlacement m on iep.IepRefID = m.IepRefID LEFT JOIN
