...
-- #############################################################################
---- Transform
IF  EXISTS (SELECT * FROM dbo.sysobjects WHERE id = OBJECT_ID(N'x_LEGACYGIFT.Transform_IepSubGoalAreaDef') AND OBJECTPROPERTY(id, N'IsView') = 1)
DROP VIEW x_LEGACYGIFT.Transform_IepSubGoalAreaDef
GO

create view x_LEGACYGIFT.Transform_IepSubGoalAreaDef
as
select 
...
x_LEGACYGIFT.MAP_IepSubGoalAreaDefID m on ks.LegacySpedCode = m.SubGoalAreaCode left join 
