...
IF EXISTS (SELECT 1 FROM sys.schemas s join sys.objects o on s.schema_id = o.schema_id where s.name = 'x_DATAVALIDATION' and o.name = 'Check_PriorWrittenNotice_Specifications')
DROP PROC x_DATAVALIDATION.Check_PriorWrittenNotice_Specifications
GO
/*
To check the specfication of Goal file with our data specification
*/
CREATE PROC x_DATAVALIDATION.Check_PriorWrittenNotice_Specifications
AS
BEGIN

DECLARE @sql nVARCHAR(MAX)

SET @sql = 'DELETE x_DATAVALIDATION.PriorWrittenNotice'
EXEC sp_executesql @stmt = @sql

---Validated data
DECLARE @sqlvalidated VARCHAR(MAX)
SET @sqlvalidated = 
'INSERT  x_DATAVALIDATION.PriorWrittenNotice  
SELECT g.Line_No '
...
FROM x_DATAVALIDATION.ValidationRules WHERE TableName = 'PriorWrittenNotice' 
...
WHILE @Count<=@MaxCount
    ...
    END
--print @sel
SET @sqlvalidated = @sqlvalidated + @sel+ ' FROM x_DATAVALIDATION.PriorWrittenNotice_Local g'
...
FROM x_DATAVALIDATION.ValidationRules WHERE TableName = 'PriorWrittenNotice' AND IsRequired =1
...
FROM x_DATAVALIDATION.ValidationRules WHERE TableName = 'PriorWrittenNotice' and IsFlagfield = 1
...
FROM x_DATAVALIDATION.ValidationRules WHERE TableName = 'PriorWrittenNotice' AND IsFkRelation = 1
SET @Count = 1
SET @Txtfkrel = ''
...
FROM x_DATAVALIDATION.ValidationRules WHERE TableName = 'PriorWrittenNotice' AND IsLookupColumn = 1
SET @Count = 1
SET @Txtlookup = ''
...
SELECT 'PriorWrittenNotice','SuccessfulRecords',COUNT(*)
FROM x_DATAVALIDATION.PriorWrittenNotice
...
SELECT ''PriorWrittenNotice'',''The field '+@columnname+' is required but not found.'',prwrn.Line_No,ISNULL(CONVERT(VARCHAR(max),st.studentLocalID),'''')+''|''+ISNULL(CONVERT(VARCHAR(max),st.FirstName),'''')+''|''+ISNULL(CONVERT(VARCHAR(max),st.LastName),'''')+''|''+ISNULL(CONVERT(VARCHAR(max),prwrn.IepRefID),'''')+''|''+ISNULL(CONVERT(VARCHAR(max),prwrn.OptionsOrFactors),'''')+''|''+ISNULL(CONVERT(VARCHAR(max),prwrn.Statement),'''') FROM x_DATAVALIDATION.PriorWrittenNotice_Local prwrn JOIN x_DATAVALIDATION.Iep_local iep ON iep.IepRefID = prwrn.IepRefID JOIN x_DATAVALIDATION.Student_local st on st.StudentrefID = iep.StudentrefID WHERE 1 = 1'

SET @query  = ' AND (prwrn.'+@columnname+' IS NULL)'
SET @vsql = @vsql + @query
--PRINT @vsql
...
SELECT ''PriorWrittenNotice'',''The field '+@columnname+' is required but not found.'', COUNT(*)
FROM x_DATAVALIDATION.PriorWrittenNotice_Local WHERE 1 = 1 '
...
SET @vsql = 'INSERT x_DATAVALIDATION.ValidationReport (TableName,ErrorMessage,LineNumber,Line)
SELECT ''PriorWrittenNotice'','''+@columnname+' field will be truncated at '+@datalength+' characters.'',prwrn.Line_No,ISNULL(CONVERT(VARCHAR(max),st.studentLocalID),'''')+''|''+ISNULL(CONVERT(VARCHAR(max),st.FirstName),'''')+''|''+ISNULL(CONVERT(VARCHAR(max),st.LastName),'''')+''|''+ISNULL(CONVERT(VARCHAR(max),prwrn.IepRefID),'''')+''|''+ISNULL(CONVERT(VARCHAR(max),prwrn.OptionsOrFactors),'''')+''|''+ISNULL(CONVERT(VARCHAR(max),prwrn.Statement),'''') FROM x_DATAVALIDATION.PriorWrittenNotice_Local prwrn JOIN x_DATAVALIDATION.Iep_local iep ON iep.IepRefID = prwrn.IepRefID JOIN x_DATAVALIDATION.Student_local st on st.StudentrefID = iep.StudentrefID WHERE 1 = 1'

SET @query  = ' AND ((DATALENGTH (REPLACE(prwrn.'+@columnname+','''''''',''''))/2) > '+@datalength+' AND prwrn.'+@columnname+' IS NOT NULL)'
SET @vsql = @vsql + @query
EXEC sp_executesql @stmt=@vsql
--PRINT @vsql

SET @sumsql = 'INSERT x_DATAVALIDATION.ValidationSummaryReport (TableName,ErrorMessage,NumberOfRecords)
SELECT ''PriorWrittenNotice'','''+@columnname+' field will be truncated at '+@datalength+' characters.'', COUNT(*)
FROM x_DATAVALIDATION.PriorWrittenNotice_Local WHERE 1 = 1 '
...
SET @vsql = 'INSERT x_DATAVALIDATION.ValidationReport (TableName,ErrorMessage,LineNumber,Line)
SELECT ''PriorWrittenNotice'',''No '+@parenttable+' record found for '+@tablename+'.'',prwrn.Line_No,ISNULL(CONVERT(VARCHAR(max),st.studentLocalID),'''')+''|''+ISNULL(CONVERT(VARCHAR(max),st.FirstName),'''')+''|''+ISNULL(CONVERT(VARCHAR(max),st.LastName),'''')+''|''+ISNULL(CONVERT(VARCHAR(max),prwrn.GoalRefID),'''')+''|''+ISNULL(CONVERT(VARCHAR(max),prwrn.IepRefID),'''')+''|''+ISNULL(CONVERT(VARCHAR(max),prwrn.Sequence),'''')+''|''+ISNULL(CONVERT(VARCHAR(max),prwrn.GoalAreaCode),'''')+''|''+ISNULL(CONVERT(VARCHAR(max),prwrn.SCInstructional),'''')+''|''+ISNULL(CONVERT(VARCHAR(max),prwrn.SCTransition),'''')+''|''+ISNULL(CONVERT(VARCHAR(max),prwrn.SCRelatedService),'''')+''|''+ISNULL(CONVERT(VARCHAR(max),prwrn.IsEsy),'''')+''|''+ISNULL(CONVERT(VARCHAR(max),prwrn.UNITOFMEASUREMENT),'''')+''|''+ISNULL(CONVERT(VARCHAR(max),prwrn.BASELINEDATAPOINT),'''')+''|''+ISNULL(CONVERT(VARCHAR(max),prwrn.EVALUATIONMETHOD),'''')+''|''+ISNULL(CONVERT(VARCHAR(max),prwrn.GoalStatement),'''')
FROM x_DATAVALIDATION.PriorWrittenNotice_Local goal JOIN x_DATAVALIDATION.Iep_local iep ON iep.IepRefID = prwrn.IepRefID JOIN x_DATAVALIDATION.Student_local st on st.StudentrefID = iep.StudentrefID '

SET @query  = ' LEFT JOIN x_DATAVALIDATION.'+@parenttable+' dt ON prwrn.'+@columnname+' = dt.'+@parentcolumn+' WHERE dt.'+@parentcolumn+' IS NULL'
SET @vsql = @vsql + @query
EXEC sp_executesql @stmt=@vsql
--PRINT @vsql

SET @sumsql = 'INSERT x_DATAVALIDATION.ValidationSummaryReport (TableName,ErrorMessage,NumberOfRecords)
SELECT ''PriorWrittenNotice'',''No '+@parenttable+' record found for '+@tablename+'.'', COUNT(*)
FROM x_DATAVALIDATION.PriorWrittenNotice_Local goal '
...
SET @vsql = 'INSERT x_DATAVALIDATION.ValidationReport (TableName,ErrorMessage,LineNumber,Line)
SELECT ''PriorWrittenNotice'',''Invalid value in '+@columnname+' it must be Y or N.'',prwrn.Line_No,ISNULL(CONVERT(VARCHAR(max),st.studentLocalID),'''')+''|''+ISNULL(CONVERT(VARCHAR(max),st.FirstName),'''')+''|''+ISNULL(CONVERT(VARCHAR(max),st.LastName),'''')+''|''+ISNULL(CONVERT(VARCHAR(max),prwrn.IepRefID),'''')+''|''+ISNULL(CONVERT(VARCHAR(max),prwrn.OptionsOrFactors),'''')+''|''+ISNULL(CONVERT(VARCHAR(max),prwrn.Statement),'''') FROM x_DATAVALIDATION.PriorWrittenNotice_Local prwrn JOIN x_DATAVALIDATION.Iep_local iep ON iep.IepRefID = prwrn.IepRefID JOIN x_DATAVALIDATION.Student_local st on st.StudentrefID = iep.StudentrefID  '

SET @query  = '  WHERE (prwrn.'+@columnname+' NOT IN  ('+@flagrecords+') AND prwrn.'+@columnname+' IS NOT NULL)'
SET @vsql = @vsql + @query
EXEC sp_executesql @stmt=@vsql
--PRINT @vsql
...
SET @sumsql = 'INSERT x_DATAVALIDATION.ValidationSummaryReport (TableName,ErrorMessage,NumberOfRecords)
SELECT ''PriorWrittenNotice'',''Invalid value in '+@columnname+' it must be Y or N.'', COUNT(*)
FROM x_DATAVALIDATION.PriorWrittenNotice_Local prwrn '
...
SET @vsql = 'INSERT x_DATAVALIDATION.ValidationReport (TableName,ErrorMessage,LineNumber,Line)
SELECT ''PriorWrittenNotice'','''+@columnname+' is duplicated.'',prwrn.Line_No,ISNULL(CONVERT(VARCHAR(max),st.studentLocalID),'''')+''|''+ISNULL(CONVERT(VARCHAR(max),st.FirstName),'''')+''|''+ISNULL(CONVERT(VARCHAR(max),st.LastName),'''')+''|''+ISNULL(CONVERT(VARCHAR(max),prwrn.IepRefID),'''')+''|''+ISNULL(CONVERT(VARCHAR(max),prwrn.OptionsOrFactors),'''')+''|''+ISNULL(CONVERT(VARCHAR(max),prwrn.Statement),'''') FROM x_DATAVALIDATION.PriorWrittenNotice_Local prwrn JOIN x_DATAVALIDATION.Iep_local iep ON iep.IepRefID = prwrn.IepRefID JOIN x_DATAVALIDATION.Student_local st on st.StudentrefID = iep.StudentrefID '

SET @query  = ' JOIN (SELECT '+@columnname+' FROM x_DATAVALIDATION.PriorWrittenNotice_Local GROUP BY '+@columnname+' HAVING COUNT(*)>1) ucprwrn ON ucprwrn.'+@columnname+' = prwrn.'+@columnname+' '
SET @vsql = @vsql + @query
EXEC sp_executesql @stmt=@vsql
--PRINT @vsql

SET @sumsql = 'INSERT x_DATAVALIDATION.ValidationSummaryReport (TableName,ErrorMessage,NumberOfRecords)
SELECT ''PriorWrittenNotice'','''+@columnname+' is duplicated.'', COUNT(*)
FROM x_DATAVALIDATION.PriorWrittenNotice_Local prwrn JOIN  '

SET @query  = ' (SELECT '+@columnname+' FROM x_DATAVALIDATION.PriorWrittenNotice_Local GROUP BY '+@columnname+' HAVING COUNT(*)>1) ucprwrn ON ucprwrn.'+@columnname+' = prwrn.'+@columnname+' '
SET @sumsql = @sumsql + @query
--PRINT @sumsql
EXEC sp_executesql @stmt=@sumsql
...
SET @vsql = 'INSERT x_DATAVALIDATION.ValidationReport (TableName,ErrorMessage,LineNumber,Line)
SELECT ''PriorWrittenNotice'',''No lookup value found for '+@columnname+'.  Contact ExcentDataTeam@excent.com .'',prwrn.Line_No,ISNULL(CONVERT(VARCHAR(max),st.studentLocalID),'''')+''|''+ISNULL(CONVERT(VARCHAR(max),st.FirstName),'''')+''|''+ISNULL(CONVERT(VARCHAR(max),st.LastName),'''')+''|''+ISNULL(CONVERT(VARCHAR(max),prwrn.IepRefID),'''')+''|''+ISNULL(CONVERT(VARCHAR(max),prwrn.OptionsOrFactors),'''')+''|''+ISNULL(CONVERT(VARCHAR(max),prwrn.Statement),'''') FROM x_DATAVALIDATION.PriorWrittenNotice_Local prwrn JOIN x_DATAVALIDATION.Iep_local iep ON iep.IepRefID = prwrn.IepRefID JOIN x_DATAVALIDATION.Student_local st on st.StudentrefID = iep.StudentrefID '

SET @query  = ' WHERE (prwrn.'+@columnname+' NOT IN ( SELECT '+@lookupcolumn+' FROM x_DATAVALIDATION.'+@lookuptable+' WHERE Type = '''+@lookuptype+''') AND prwrn.'+@columnname+' IS NOT NULL)'
SET @vsql = @vsql + @query
EXEC sp_executesql @stmt=@vsql
--PRINT @vsql

SET @sumsql = 'INSERT x_DATAVALIDATION.ValidationSummaryReport (TableName,ErrorMessage,NumberOfRecords)
SELECT ''PriorWrittenNotice'',''No lookup value found for '+@columnname+'.'', COUNT(*)
FROM x_DATAVALIDATION.PriorWrittenNotice_Local prwrn  '
...
SET @query  = ' WHERE (prwrn.'+@columnname+' NOT IN ( SELECT '+@lookupcolumn+' FROM x_DATAVALIDATION.'+@lookuptable+') AND prwrn.'+@columnname+' IS NOT NULL)'
SET @vsql = @vsql + @query
EXEC sp_executesql @stmt=@vsql
--PRINT @vsql
...
SET @vsql = 'INSERT x_DATAVALIDATION.ValidationReport (TableName,ErrorMessage,LineNumber,Line)
SELECT ''PriorWrittenNotice'',''Expected date value in '+@columnname+'.'',prwrn.Line_No,ISNULL(CONVERT(VARCHAR(max),st.studentLocalID),'''')+''|''+ISNULL(CONVERT(VARCHAR(max),st.FirstName),'''')+''|''+ISNULL(CONVERT(VARCHAR(max),st.LastName),'''')+''|''+ISNULL(CONVERT(VARCHAR(max),prwrn.IepRefID),'''')+''|''+ISNULL(CONVERT(VARCHAR(max),prwrn.OptionsOrFactors),'''')+''|''+ISNULL(CONVERT(VARCHAR(max),prwrn.Statement),'''') FROM x_DATAVALIDATION.PriorWrittenNotice_Local prwrn JOIN x_DATAVALIDATION.Iep_local iep ON iep.IepRefID = prwrn.IepRefID JOIN x_DATAVALIDATION.Student_local st on st.StudentrefID = iep.StudentrefID WHERE 1 = 1 '

SET @query  = ' AND (ISDATE(''prwrn.'+@columnname+''') = 0 AND prwrn.'+@columnname+' IS NOT NULL)'
SET @vsql = @vsql + @query
EXEC sp_executesql @stmt=@vsql
--PRINT @vsql

SET @sumsql = 'INSERT x_DATAVALIDATION.ValidationSummaryReport (TableName,ErrorMessage,NumberOfRecords)
SELECT ''PriorWrittenNotice'',''Expected date value in '+@columnname+'.'', COUNT(*)
FROM x_DATAVALIDATION.PriorWrittenNotice_Local WHERE 1 = 1 '
...
SET @vsql = 'INSERT x_DATAVALIDATION.ValidationReport (TableName,ErrorMessage,LineNumber,Line)
SELECT ''PriorWrittenNotice'',''Expected integer value in '+@columnname+'.'',prwrn.Line_No,ISNULL(CONVERT(VARCHAR(max),st.studentLocalID),'''')+''|''+ISNULL(CONVERT(VARCHAR(max),st.FirstName),'''')+''|''+ISNULL(CONVERT(VARCHAR(max),st.LastName),'''')+''|''+ISNULL(CONVERT(VARCHAR(max),prwrn.IepRefID),'''')+''|''+ISNULL(CONVERT(VARCHAR(max),prwrn.OptionsOrFactors),'''')+''|''+ISNULL(CONVERT(VARCHAR(max),prwrn.Statement),'''') FROM x_DATAVALIDATION.PriorWrittenNotice_Local prwrn JOIN x_DATAVALIDATION.Iep_local iep ON iep.IepRefID = prwrn.IepRefID JOIN x_DATAVALIDATION.Student_local st on st.StudentrefID = iep.StudentrefID WHERE 1 = 1 '

SET @query  = ' AND (x_DATAVALIDATION.udf_IsInteger(prwrn.'+@columnname+') = 0 AND prwrn.'+@columnname+' IS NOT NULL)'
SET @vsql = @vsql + @query
EXEC sp_executesql @stmt=@vsql
--PRINT @vsql

SET @sumsql = 'INSERT x_DATAVALIDATION.ValidationSummaryReport (TableName,ErrorMessage,NumberOfRecords)
SELECT ''PriorWrittenNotice'',''Expected integer value in '+@columnname+'.'', COUNT(*)
FROM x_DATAVALIDATION.PriorWrittenNotice_Local WHERE 1 = 1 '
