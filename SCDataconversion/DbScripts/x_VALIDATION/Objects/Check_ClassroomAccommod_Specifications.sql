...
IF EXISTS (SELECT 1 FROM sys.schemas s join sys.objects o on s.schema_id = o.schema_id where s.name = 'x_DATAVALIDATION' and o.name = 'Check_ClassroomAccommMod_Specifications')
DROP PROC x_DATAVALIDATION.Check_ClassroomAccommMod_Specifications
GO
/*
To check the specfication of Goal file with our data specification
*/
CREATE PROC x_DATAVALIDATION.Check_ClassroomAccommMod_Specifications
AS
BEGIN

DECLARE @sql nVARCHAR(MAX)

SET @sql = 'DELETE x_DATAVALIDATION.ClassroomAccommMod'
EXEC sp_executesql @stmt = @sql

---Validated data
DECLARE @sqlvalidated VARCHAR(MAX)
SET @sqlvalidated = 
'INSERT  x_DATAVALIDATION.ClassroomAccommMod  
SELECT g.Line_No '
...
FROM x_DATAVALIDATION.ValidationRules WHERE TableName = 'ClassroomAccommMod' 
...
WHILE @Count<=@MaxCount
    ...
    END
--print @sel
SET @sqlvalidated = @sqlvalidated + @sel+ ' FROM x_DATAVALIDATION.ClassroomAccommMod_Local g'
...
FROM x_DATAVALIDATION.ValidationRules WHERE TableName = 'ClassroomAccommMod' AND IsRequired =1
...
FROM x_DATAVALIDATION.ValidationRules WHERE TableName = 'ClassroomAccommMod' and IsFlagfield = 1
...
FROM x_DATAVALIDATION.ValidationRules WHERE TableName = 'ClassroomAccommMod' AND IsFkRelation = 1
SET @Count = 1
SET @Txtfkrel = ''
...
FROM x_DATAVALIDATION.ValidationRules WHERE TableName = 'ClassroomAccommMod' AND IsLookupColumn = 1
SET @Count = 1
SET @Txtlookup = ''
...
SELECT 'ClassroomAccommMod','SuccessfulRecords',COUNT(*)
FROM x_DATAVALIDATION.ClassroomAccommMod
...
SELECT ''ClassroomAccommMod'',''The field '+@columnname+' is required but not found.'',clamod.Line_No,ISNULL(CONVERT(VARCHAR(max),st.studentLocalID),'''')+''|''+ISNULL(CONVERT(VARCHAR(max),st.FirstName),'''')+''|''+ISNULL(CONVERT(VARCHAR(max),st.LastName),'''')+''|''+ISNULL(CONVERT(VARCHAR(max),clamod.IepRefID),'''')+''|''+ISNULL(CONVERT(VARCHAR(max),clamod.AccommOrMod),'''')+''|''+ISNULL(CONVERT(VARCHAR(max),clamod.Statement),'''') FROM x_DATAVALIDATION.ClassroomAccommMod_Local clamod JOIN x_DATAVALIDATION.Iep_local iep ON iep.IepRefID = clamod.IepRefID JOIN x_DATAVALIDATION.Student_local st on st.StudentrefID = iep.StudentrefID WHERE 1 = 1'

SET @query  = ' AND (clamod.'+@columnname+' IS NULL)'
SET @vsql = @vsql + @query
--PRINT @vsql
...
SELECT ''ClassroomAccommMod'',''The field '+@columnname+' is required but not found.'', COUNT(*)
FROM x_DATAVALIDATION.ClassroomAccommMod_Local WHERE 1 = 1 '
...
SET @vsql = 'INSERT x_DATAVALIDATION.ValidationReport (TableName,ErrorMessage,LineNumber,Line)
SELECT ''ClassroomAccommMod'','''+@columnname+' field will be truncated at '+@datalength+' characters.'',clamod.Line_No,ISNULL(CONVERT(VARCHAR(max),st.studentLocalID),'''')+''|''+ISNULL(CONVERT(VARCHAR(max),st.FirstName),'''')+''|''+ISNULL(CONVERT(VARCHAR(max),st.LastName),'''')+''|''+ISNULL(CONVERT(VARCHAR(max),clamod.IepRefID),'''')+''|''+ISNULL(CONVERT(VARCHAR(max),clamod.AccommOrMod),'''')+''|''+ISNULL(CONVERT(VARCHAR(max),clamod.Statement),'''') FROM x_DATAVALIDATION.ClassroomAccommMod_Local clamod JOIN x_DATAVALIDATION.Iep_local iep ON iep.IepRefID = clamod.IepRefID JOIN x_DATAVALIDATION.Student_local st on st.StudentrefID = iep.StudentrefID WHERE 1 = 1'

SET @query  = ' AND ((DATALENGTH (REPLACE(clamod.'+@columnname+','''''''',''''))/2) > '+@datalength+' AND clamod.'+@columnname+' IS NOT NULL)'
SET @vsql = @vsql + @query
EXEC sp_executesql @stmt=@vsql
--PRINT @vsql

SET @sumsql = 'INSERT x_DATAVALIDATION.ValidationSummaryReport (TableName,ErrorMessage,NumberOfRecords)
SELECT ''ClassroomAccommMod'','''+@columnname+' field will be truncated at '+@datalength+' characters.'', COUNT(*)
FROM x_DATAVALIDATION.ClassroomAccommMod_Local WHERE 1 = 1 '
...
SET @vsql = 'INSERT x_DATAVALIDATION.ValidationReport (TableName,ErrorMessage,LineNumber,Line)
SELECT ''ClassroomAccommMod'',''No '+@parenttable+' record found for '+@tablename+'.'',clamod.Line_No,ISNULL(CONVERT(VARCHAR(max),st.studentLocalID),'''')+''|''+ISNULL(CONVERT(VARCHAR(max),st.FirstName),'''')+''|''+ISNULL(CONVERT(VARCHAR(max),st.LastName),'''')+''|''+ISNULL(CONVERT(VARCHAR(max),clamod.GoalRefID),'''')+''|''+ISNULL(CONVERT(VARCHAR(max),clamod.IepRefID),'''')+''|''+ISNULL(CONVERT(VARCHAR(max),clamod.Sequence),'''')+''|''+ISNULL(CONVERT(VARCHAR(max),clamod.GoalAreaCode),'''')+''|''+ISNULL(CONVERT(VARCHAR(max),clamod.SCInstructional),'''')+''|''+ISNULL(CONVERT(VARCHAR(max),clamod.SCTransition),'''')+''|''+ISNULL(CONVERT(VARCHAR(max),clamod.SCRelatedService),'''')+''|''+ISNULL(CONVERT(VARCHAR(max),clamod.IsEsy),'''')+''|''+ISNULL(CONVERT(VARCHAR(max),clamod.UNITOFMEASUREMENT),'''')+''|''+ISNULL(CONVERT(VARCHAR(max),clamod.BASELINEDATAPOINT),'''')+''|''+ISNULL(CONVERT(VARCHAR(max),clamod.EVALUATIONMETHOD),'''')+''|''+ISNULL(CONVERT(VARCHAR(max),clamod.GoalStatement),'''')
FROM x_DATAVALIDATION.ClassroomAccommMod_Local goal JOIN x_DATAVALIDATION.Iep_local iep ON iep.IepRefID = clamod.IepRefID JOIN x_DATAVALIDATION.Student_local st on st.StudentrefID = iep.StudentrefID '

SET @query  = ' LEFT JOIN x_DATAVALIDATION.'+@parenttable+' dt ON clamod.'+@columnname+' = dt.'+@parentcolumn+' WHERE dt.'+@parentcolumn+' IS NULL'
SET @vsql = @vsql + @query
EXEC sp_executesql @stmt=@vsql
--PRINT @vsql

SET @sumsql = 'INSERT x_DATAVALIDATION.ValidationSummaryReport (TableName,ErrorMessage,NumberOfRecords)
SELECT ''ClassroomAccommMod'',''No '+@parenttable+' record found for '+@tablename+'.'', COUNT(*)
FROM x_DATAVALIDATION.ClassroomAccommMod_Local goal '
...
SET @vsql = 'INSERT x_DATAVALIDATION.ValidationReport (TableName,ErrorMessage,LineNumber,Line)
SELECT ''ClassroomAccommMod'',''Invalid value in '+@columnname+' it must be Y or N.'',clamod.Line_No,ISNULL(CONVERT(VARCHAR(max),st.studentLocalID),'''')+''|''+ISNULL(CONVERT(VARCHAR(max),st.FirstName),'''')+''|''+ISNULL(CONVERT(VARCHAR(max),st.LastName),'''')+''|''+ISNULL(CONVERT(VARCHAR(max),clamod.IepRefID),'''')+''|''+ISNULL(CONVERT(VARCHAR(max),clamod.AccommOrMod),'''')+''|''+ISNULL(CONVERT(VARCHAR(max),clamod.Statement),'''') FROM x_DATAVALIDATION.ClassroomAccommMod_Local clamod JOIN x_DATAVALIDATION.Iep_local iep ON iep.IepRefID = clamod.IepRefID JOIN x_DATAVALIDATION.Student_local st on st.StudentrefID = iep.StudentrefID  '

SET @query  = '  WHERE (clamod.'+@columnname+' NOT IN  ('+@flagrecords+') AND clamod.'+@columnname+' IS NOT NULL)'
SET @vsql = @vsql + @query
EXEC sp_executesql @stmt=@vsql
--PRINT @vsql
...
SET @sumsql = 'INSERT x_DATAVALIDATION.ValidationSummaryReport (TableName,ErrorMessage,NumberOfRecords)
SELECT ''ClassroomAccommMod'',''Invalid value in '+@columnname+' it must be Y or N.'', COUNT(*)
FROM x_DATAVALIDATION.ClassroomAccommMod_Local clamod '
...
SET @vsql = 'INSERT x_DATAVALIDATION.ValidationReport (TableName,ErrorMessage,LineNumber,Line)
SELECT ''ClassroomAccommMod'','''+@columnname+' is duplicated.'',clamod.Line_No,ISNULL(CONVERT(VARCHAR(max),st.studentLocalID),'''')+''|''+ISNULL(CONVERT(VARCHAR(max),st.FirstName),'''')+''|''+ISNULL(CONVERT(VARCHAR(max),st.LastName),'''')+''|''+ISNULL(CONVERT(VARCHAR(max),clamod.IepRefID),'''')+''|''+ISNULL(CONVERT(VARCHAR(max),clamod.AccommOrMod),'''')+''|''+ISNULL(CONVERT(VARCHAR(max),clamod.Statement),'''') FROM x_DATAVALIDATION.ClassroomAccommMod_Local clamod JOIN x_DATAVALIDATION.Iep_local iep ON iep.IepRefID = clamod.IepRefID JOIN x_DATAVALIDATION.Student_local st on st.StudentrefID = iep.StudentrefID '

SET @query  = ' JOIN (SELECT '+@columnname+' FROM x_DATAVALIDATION.ClassroomAccommMod_Local GROUP BY '+@columnname+' HAVING COUNT(*)>1) ucclamod ON ucclamod.'+@columnname+' = clamod.'+@columnname+' '
SET @vsql = @vsql + @query
EXEC sp_executesql @stmt=@vsql
--PRINT @vsql

SET @sumsql = 'INSERT x_DATAVALIDATION.ValidationSummaryReport (TableName,ErrorMessage,NumberOfRecords)
SELECT ''ClassroomAccommMod'','''+@columnname+' is duplicated.'', COUNT(*)
FROM x_DATAVALIDATION.ClassroomAccommMod_Local clamod JOIN  '

SET @query  = ' (SELECT '+@columnname+' FROM x_DATAVALIDATION.ClassroomAccommMod_Local GROUP BY '+@columnname+' HAVING COUNT(*)>1) ucclamod ON ucclamod.'+@columnname+' = clamod.'+@columnname+' '
SET @sumsql = @sumsql + @query
--PRINT @sumsql
EXEC sp_executesql @stmt=@sumsql
...
SET @vsql = 'INSERT x_DATAVALIDATION.ValidationReport (TableName,ErrorMessage,LineNumber,Line)
SELECT ''ClassroomAccommMod'',''No lookup value found for '+@columnname+'.  Contact ExcentDataTeam@excent.com .'',clamod.Line_No,ISNULL(CONVERT(VARCHAR(max),st.studentLocalID),'''')+''|''+ISNULL(CONVERT(VARCHAR(max),st.FirstName),'''')+''|''+ISNULL(CONVERT(VARCHAR(max),st.LastName),'''')+''|''+ISNULL(CONVERT(VARCHAR(max),clamod.IepRefID),'''')+''|''+ISNULL(CONVERT(VARCHAR(max),clamod.AccommOrMod),'''')+''|''+ISNULL(CONVERT(VARCHAR(max),clamod.Statement),'''') FROM x_DATAVALIDATION.ClassroomAccommMod_Local clamod JOIN x_DATAVALIDATION.Iep_local iep ON iep.IepRefID = clamod.IepRefID JOIN x_DATAVALIDATION.Student_local st on st.StudentrefID = iep.StudentrefID '

SET @query  = ' WHERE (clamod.'+@columnname+' NOT IN ( SELECT '+@lookupcolumn+' FROM x_DATAVALIDATION.'+@lookuptable+' WHERE Type = '''+@lookuptype+''') AND clamod.'+@columnname+' IS NOT NULL)'
SET @vsql = @vsql + @query
EXEC sp_executesql @stmt=@vsql
--PRINT @vsql

SET @sumsql = 'INSERT x_DATAVALIDATION.ValidationSummaryReport (TableName,ErrorMessage,NumberOfRecords)
SELECT ''ClassroomAccommMod'',''No lookup value found for '+@columnname+'.'', COUNT(*)
FROM x_DATAVALIDATION.ClassroomAccommMod_Local clamod  '
...
SET @query  = ' WHERE (clamod.'+@columnname+' NOT IN ( SELECT '+@lookupcolumn+' FROM x_DATAVALIDATION.'+@lookuptable+') AND clamod.'+@columnname+' IS NOT NULL)'
SET @vsql = @vsql + @query
EXEC sp_executesql @stmt=@vsql
--PRINT @vsql
...
SET @vsql = 'INSERT x_DATAVALIDATION.ValidationReport (TableName,ErrorMessage,LineNumber,Line)
SELECT ''ClassroomAccommMod'',''Expected date value in '+@columnname+'.'',clamod.Line_No,ISNULL(CONVERT(VARCHAR(max),st.studentLocalID),'''')+''|''+ISNULL(CONVERT(VARCHAR(max),st.FirstName),'''')+''|''+ISNULL(CONVERT(VARCHAR(max),st.LastName),'''')+''|''+ISNULL(CONVERT(VARCHAR(max),clamod.IepRefID),'''')+''|''+ISNULL(CONVERT(VARCHAR(max),clamod.AccommOrMod),'''')+''|''+ISNULL(CONVERT(VARCHAR(max),clamod.Statement),'''') FROM x_DATAVALIDATION.ClassroomAccommMod_Local clamod JOIN x_DATAVALIDATION.Iep_local iep ON iep.IepRefID = clamod.IepRefID JOIN x_DATAVALIDATION.Student_local st on st.StudentrefID = iep.StudentrefID WHERE 1 = 1 '

SET @query  = ' AND (ISDATE(''clamod.'+@columnname+''') = 0 AND clamod.'+@columnname+' IS NOT NULL)'
SET @vsql = @vsql + @query
EXEC sp_executesql @stmt=@vsql
--PRINT @vsql

SET @sumsql = 'INSERT x_DATAVALIDATION.ValidationSummaryReport (TableName,ErrorMessage,NumberOfRecords)
SELECT ''ClassroomAccommMod'',''Expected date value in '+@columnname+'.'', COUNT(*)
FROM x_DATAVALIDATION.ClassroomAccommMod_Local WHERE 1 = 1 '
...
SET @vsql = 'INSERT x_DATAVALIDATION.ValidationReport (TableName,ErrorMessage,LineNumber,Line)
SELECT ''ClassroomAccommMod'',''Expected integer value in '+@columnname+'.'',clamod.Line_No,ISNULL(CONVERT(VARCHAR(max),st.studentLocalID),'''')+''|''+ISNULL(CONVERT(VARCHAR(max),st.FirstName),'''')+''|''+ISNULL(CONVERT(VARCHAR(max),st.LastName),'''')+''|''+ISNULL(CONVERT(VARCHAR(max),clamod.IepRefID),'''')+''|''+ISNULL(CONVERT(VARCHAR(max),clamod.AccommOrMod),'''')+''|''+ISNULL(CONVERT(VARCHAR(max),clamod.Statement),'''') FROM x_DATAVALIDATION.ClassroomAccommMod_Local clamod JOIN x_DATAVALIDATION.Iep_local iep ON iep.IepRefID = clamod.IepRefID JOIN x_DATAVALIDATION.Student_local st on st.StudentrefID = iep.StudentrefID WHERE 1 = 1 '

SET @query  = ' AND (x_DATAVALIDATION.udf_IsInteger(clamod.'+@columnname+') = 0 AND clamod.'+@columnname+' IS NOT NULL)'
SET @vsql = @vsql + @query
EXEC sp_executesql @stmt=@vsql
--PRINT @vsql

SET @sumsql = 'INSERT x_DATAVALIDATION.ValidationSummaryReport (TableName,ErrorMessage,NumberOfRecords)
SELECT ''ClassroomAccommMod'',''Expected integer value in '+@columnname+'.'', COUNT(*)
FROM x_DATAVALIDATION.ClassroomAccommMod_Local WHERE 1 = 1 '
