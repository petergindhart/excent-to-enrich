...
IF EXISTS (SELECT 1 FROM sys.schemas s join sys.objects o on s.schema_id = o.schema_id where s.name = 'x_DATAVALIDATION' and o.name = 'Check_PresentLevel_Specifications')
DROP PROC x_DATAVALIDATION.Check_PresentLevel_Specifications
GO
/*
To check the specfication of Goal file with our data specification
*/
CREATE PROC x_DATAVALIDATION.Check_PresentLevel_Specifications
AS
BEGIN

DECLARE @sql nVARCHAR(MAX)

SET @sql = 'DELETE x_DATAVALIDATION.PresentLevel'
EXEC sp_executesql @stmt = @sql

---Validated data
DECLARE @sqlvalidated VARCHAR(MAX)
SET @sqlvalidated = 
'INSERT  x_DATAVALIDATION.PresentLevel  
SELECT g.Line_No '
...
FROM x_DATAVALIDATION.ValidationRules WHERE TableName = 'PresentLevel' 
...
WHILE @Count<=@MaxCount
    ...
    END
--print @sel
SET @sqlvalidated = @sqlvalidated + @sel+ ' FROM x_DATAVALIDATION.PresentLevel_Local g'
...
FROM x_DATAVALIDATION.ValidationRules WHERE TableName = 'PresentLevel' AND IsRequired =1
...
FROM x_DATAVALIDATION.ValidationRules WHERE TableName = 'PresentLevel' and IsFlagfield = 1
...
FROM x_DATAVALIDATION.ValidationRules WHERE TableName = 'PresentLevel' AND IsFkRelation = 1
SET @Count = 1
SET @Txtfkrel = ''
...
FROM x_DATAVALIDATION.ValidationRules WHERE TableName = 'PresentLevel' AND IsLookupColumn = 1
SET @Count = 1
SET @Txtlookup = ''
...
SELECT 'PresentLevel','SuccessfulRecords',COUNT(*)
FROM x_DATAVALIDATION.PresentLevel
...
SELECT ''PresentLevel'',''The field '+@columnname+' is required but not found.'',prlevel.Line_No,ISNULL(CONVERT(VARCHAR(max),st.studentLocalID),'''')+''|''+ISNULL(CONVERT(VARCHAR(max),st.FirstName),'''')+''|''+ISNULL(CONVERT(VARCHAR(max),st.LastName),'''')+''|''+ISNULL(CONVERT(VARCHAR(max),prlevel.IepRefID),'''')+''|''+ISNULL(CONVERT(VARCHAR(max),prlevel.PresentLevelType),'''')+''|''+ISNULL(CONVERT(VARCHAR(max),prlevel.Statement),'''') FROM x_DATAVALIDATION.PresentLevel_Local prlevel JOIN x_DATAVALIDATION.Iep_local iep ON iep.IepRefID = prlevel.IepRefID JOIN x_DATAVALIDATION.Student_local st on st.StudentrefID = iep.StudentrefID WHERE 1 = 1'

SET @query  = ' AND (prlevel.'+@columnname+' IS NULL)'
SET @vsql = @vsql + @query
--PRINT @vsql
...
SELECT ''PresentLevel'',''The field '+@columnname+' is required but not found.'', COUNT(*)
FROM x_DATAVALIDATION.PresentLevel_Local WHERE 1 = 1 '
...
SET @vsql = 'INSERT x_DATAVALIDATION.ValidationReport (TableName,ErrorMessage,LineNumber,Line)
SELECT ''PresentLevel'','''+@columnname+' field will be truncated at '+@datalength+' characters.'',prlevel.Line_No,ISNULL(CONVERT(VARCHAR(max),st.studentLocalID),'''')+''|''+ISNULL(CONVERT(VARCHAR(max),st.FirstName),'''')+''|''+ISNULL(CONVERT(VARCHAR(max),st.LastName),'''')+''|''+ISNULL(CONVERT(VARCHAR(max),prlevel.IepRefID),'''')+''|''+ISNULL(CONVERT(VARCHAR(max),prlevel.PresentLevelType),'''')+''|''+ISNULL(CONVERT(VARCHAR(max),prlevel.Statement),'''') FROM x_DATAVALIDATION.PresentLevel_Local prlevel JOIN x_DATAVALIDATION.Iep_local iep ON iep.IepRefID = prlevel.IepRefID JOIN x_DATAVALIDATION.Student_local st on st.StudentrefID = iep.StudentrefID WHERE 1 = 1'

SET @query  = ' AND ((DATALENGTH (REPLACE(prlevel.'+@columnname+','''''''',''''))/2) > '+@datalength+' AND prlevel.'+@columnname+' IS NOT NULL)'
SET @vsql = @vsql + @query
EXEC sp_executesql @stmt=@vsql
--PRINT @vsql

SET @sumsql = 'INSERT x_DATAVALIDATION.ValidationSummaryReport (TableName,ErrorMessage,NumberOfRecords)
SELECT ''PresentLevel'','''+@columnname+' field will be truncated at '+@datalength+' characters.'', COUNT(*)
FROM x_DATAVALIDATION.PresentLevel_Local WHERE 1 = 1 '
...
SET @vsql = 'INSERT x_DATAVALIDATION.ValidationReport (TableName,ErrorMessage,LineNumber,Line)
SELECT ''PresentLevel'',''No '+@parenttable+' record found for '+@tablename+'.'',prlevel.Line_No,ISNULL(CONVERT(VARCHAR(max),st.studentLocalID),'''')+''|''+ISNULL(CONVERT(VARCHAR(max),st.FirstName),'''')+''|''+ISNULL(CONVERT(VARCHAR(max),st.LastName),'''')+''|''+ISNULL(CONVERT(VARCHAR(max),prlevel.GoalRefID),'''')+''|''+ISNULL(CONVERT(VARCHAR(max),prlevel.IepRefID),'''')+''|''+ISNULL(CONVERT(VARCHAR(max),prlevel.Sequence),'''')+''|''+ISNULL(CONVERT(VARCHAR(max),prlevel.GoalAreaCode),'''')+''|''+ISNULL(CONVERT(VARCHAR(max),prlevel.SCInstructional),'''')+''|''+ISNULL(CONVERT(VARCHAR(max),prlevel.SCTransition),'''')+''|''+ISNULL(CONVERT(VARCHAR(max),prlevel.SCRelatedService),'''')+''|''+ISNULL(CONVERT(VARCHAR(max),prlevel.IsEsy),'''')+''|''+ISNULL(CONVERT(VARCHAR(max),prlevel.UNITOFMEASUREMENT),'''')+''|''+ISNULL(CONVERT(VARCHAR(max),prlevel.BASELINEDATAPOINT),'''')+''|''+ISNULL(CONVERT(VARCHAR(max),prlevel.EVALUATIONMETHOD),'''')+''|''+ISNULL(CONVERT(VARCHAR(max),prlevel.GoalStatement),'''')
FROM x_DATAVALIDATION.PresentLevel_Local goal JOIN x_DATAVALIDATION.Iep_local iep ON iep.IepRefID = prlevel.IepRefID JOIN x_DATAVALIDATION.Student_local st on st.StudentrefID = iep.StudentrefID '

SET @query  = ' LEFT JOIN x_DATAVALIDATION.'+@parenttable+' dt ON prlevel.'+@columnname+' = dt.'+@parentcolumn+' WHERE dt.'+@parentcolumn+' IS NULL'
SET @vsql = @vsql + @query
EXEC sp_executesql @stmt=@vsql
--PRINT @vsql

SET @sumsql = 'INSERT x_DATAVALIDATION.ValidationSummaryReport (TableName,ErrorMessage,NumberOfRecords)
SELECT ''PresentLevel'',''No '+@parenttable+' record found for '+@tablename+'.'', COUNT(*)
FROM x_DATAVALIDATION.PresentLevel_Local goal '
...
SET @vsql = 'INSERT x_DATAVALIDATION.ValidationReport (TableName,ErrorMessage,LineNumber,Line)
SELECT ''PresentLevel'',''Invalid value in '+@columnname+' it must be Y or N.'',prlevel.Line_No,ISNULL(CONVERT(VARCHAR(max),st.studentLocalID),'''')+''|''+ISNULL(CONVERT(VARCHAR(max),st.FirstName),'''')+''|''+ISNULL(CONVERT(VARCHAR(max),st.LastName),'''')+''|''+ISNULL(CONVERT(VARCHAR(max),prlevel.IepRefID),'''')+''|''+ISNULL(CONVERT(VARCHAR(max),prlevel.PresentLevelType),'''')+''|''+ISNULL(CONVERT(VARCHAR(max),prlevel.Statement),'''') FROM x_DATAVALIDATION.PresentLevel_Local prlevel JOIN x_DATAVALIDATION.Iep_local iep ON iep.IepRefID = prlevel.IepRefID JOIN x_DATAVALIDATION.Student_local st on st.StudentrefID = iep.StudentrefID  '

SET @query  = '  WHERE (prlevel.'+@columnname+' NOT IN  ('+@flagrecords+') AND prlevel.'+@columnname+' IS NOT NULL)'
SET @vsql = @vsql + @query
EXEC sp_executesql @stmt=@vsql
--PRINT @vsql
...
SET @sumsql = 'INSERT x_DATAVALIDATION.ValidationSummaryReport (TableName,ErrorMessage,NumberOfRecords)
SELECT ''PresentLevel'',''Invalid value in '+@columnname+' it must be Y or N.'', COUNT(*)
FROM x_DATAVALIDATION.PresentLevel_Local prlevel '
...
SET @vsql = 'INSERT x_DATAVALIDATION.ValidationReport (TableName,ErrorMessage,LineNumber,Line)
SELECT ''PresentLevel'','''+@columnname+' is duplicated.'',prlevel.Line_No,ISNULL(CONVERT(VARCHAR(max),st.studentLocalID),'''')+''|''+ISNULL(CONVERT(VARCHAR(max),st.FirstName),'''')+''|''+ISNULL(CONVERT(VARCHAR(max),st.LastName),'''')+''|''+ISNULL(CONVERT(VARCHAR(max),prlevel.IepRefID),'''')+''|''+ISNULL(CONVERT(VARCHAR(max),prlevel.PresentLevelType),'''')+''|''+ISNULL(CONVERT(VARCHAR(max),prlevel.Statement),'''') FROM x_DATAVALIDATION.PresentLevel_Local prlevel JOIN x_DATAVALIDATION.Iep_local iep ON iep.IepRefID = prlevel.IepRefID JOIN x_DATAVALIDATION.Student_local st on st.StudentrefID = iep.StudentrefID '

SET @query  = ' JOIN (SELECT '+@columnname+' FROM x_DATAVALIDATION.PresentLevel_Local GROUP BY '+@columnname+' HAVING COUNT(*)>1) ucprlevel ON ucprlevel.'+@columnname+' = prlevel.'+@columnname+' '
SET @vsql = @vsql + @query
EXEC sp_executesql @stmt=@vsql
--PRINT @vsql

SET @sumsql = 'INSERT x_DATAVALIDATION.ValidationSummaryReport (TableName,ErrorMessage,NumberOfRecords)
SELECT ''PresentLevel'','''+@columnname+' is duplicated.'', COUNT(*)
FROM x_DATAVALIDATION.PresentLevel_Local prlevel JOIN  '

SET @query  = ' (SELECT '+@columnname+' FROM x_DATAVALIDATION.PresentLevel_Local GROUP BY '+@columnname+' HAVING COUNT(*)>1) ucprlevel ON ucprlevel.'+@columnname+' = prlevel.'+@columnname+' '
SET @sumsql = @sumsql + @query
--PRINT @sumsql
EXEC sp_executesql @stmt=@sumsql
...
SET @vsql = 'INSERT x_DATAVALIDATION.ValidationReport (TableName,ErrorMessage,LineNumber,Line)
SELECT ''PresentLevel'',''No lookup value found for '+@columnname+'.  Contact ExcentDataTeam@excent.com .'',prlevel.Line_No,ISNULL(CONVERT(VARCHAR(max),st.studentLocalID),'''')+''|''+ISNULL(CONVERT(VARCHAR(max),st.FirstName),'''')+''|''+ISNULL(CONVERT(VARCHAR(max),st.LastName),'''')+''|''+ISNULL(CONVERT(VARCHAR(max),prlevel.IepRefID),'''')+''|''+ISNULL(CONVERT(VARCHAR(max),prlevel.PresentLevelType),'''')+''|''+ISNULL(CONVERT(VARCHAR(max),prlevel.Statement),'''') FROM x_DATAVALIDATION.PresentLevel_Local prlevel JOIN x_DATAVALIDATION.Iep_local iep ON iep.IepRefID = prlevel.IepRefID JOIN x_DATAVALIDATION.Student_local st on st.StudentrefID = iep.StudentrefID '

SET @query  = ' WHERE (prlevel.'+@columnname+' NOT IN ( SELECT '+@lookupcolumn+' FROM x_DATAVALIDATION.'+@lookuptable+' WHERE Type = '''+@lookuptype+''') AND prlevel.'+@columnname+' IS NOT NULL)'
SET @vsql = @vsql + @query
EXEC sp_executesql @stmt=@vsql
--PRINT @vsql

SET @sumsql = 'INSERT x_DATAVALIDATION.ValidationSummaryReport (TableName,ErrorMessage,NumberOfRecords)
SELECT ''PresentLevel'',''No lookup value found for '+@columnname+'.'', COUNT(*)
FROM x_DATAVALIDATION.PresentLevel_Local prlevel  '
...
SET @query  = ' WHERE (prlevel.'+@columnname+' NOT IN ( SELECT '+@lookupcolumn+' FROM x_DATAVALIDATION.'+@lookuptable+') AND prlevel.'+@columnname+' IS NOT NULL)'
SET @vsql = @vsql + @query
EXEC sp_executesql @stmt=@vsql
--PRINT @vsql
...
SET @vsql = 'INSERT x_DATAVALIDATION.ValidationReport (TableName,ErrorMessage,LineNumber,Line)
SELECT ''PresentLevel'',''Expected date value in '+@columnname+'.'',prlevel.Line_No,ISNULL(CONVERT(VARCHAR(max),st.studentLocalID),'''')+''|''+ISNULL(CONVERT(VARCHAR(max),st.FirstName),'''')+''|''+ISNULL(CONVERT(VARCHAR(max),st.LastName),'''')+''|''+ISNULL(CONVERT(VARCHAR(max),prlevel.IepRefID),'''')+''|''+ISNULL(CONVERT(VARCHAR(max),prlevel.PresentLevelType),'''')+''|''+ISNULL(CONVERT(VARCHAR(max),prlevel.Statement),'''') FROM x_DATAVALIDATION.PresentLevel_Local prlevel JOIN x_DATAVALIDATION.Iep_local iep ON iep.IepRefID = prlevel.IepRefID JOIN x_DATAVALIDATION.Student_local st on st.StudentrefID = iep.StudentrefID WHERE 1 = 1 '

SET @query  = ' AND (ISDATE(''prlevel.'+@columnname+''') = 0 AND prlevel.'+@columnname+' IS NOT NULL)'
SET @vsql = @vsql + @query
EXEC sp_executesql @stmt=@vsql
--PRINT @vsql

SET @sumsql = 'INSERT x_DATAVALIDATION.ValidationSummaryReport (TableName,ErrorMessage,NumberOfRecords)
SELECT ''PresentLevel'',''Expected date value in '+@columnname+'.'', COUNT(*)
FROM x_DATAVALIDATION.PresentLevel_Local WHERE 1 = 1 '
...
SET @vsql = 'INSERT x_DATAVALIDATION.ValidationReport (TableName,ErrorMessage,LineNumber,Line)
SELECT ''PresentLevel'',''Expected integer value in '+@columnname+'.'',prlevel.Line_No,ISNULL(CONVERT(VARCHAR(max),st.studentLocalID),'''')+''|''+ISNULL(CONVERT(VARCHAR(max),st.FirstName),'''')+''|''+ISNULL(CONVERT(VARCHAR(max),st.LastName),'''')+''|''+ISNULL(CONVERT(VARCHAR(max),prlevel.IepRefID),'''')+''|''+ISNULL(CONVERT(VARCHAR(max),prlevel.PresentLevelType),'''')+''|''+ISNULL(CONVERT(VARCHAR(max),prlevel.Statement),'''') FROM x_DATAVALIDATION.PresentLevel_Local prlevel JOIN x_DATAVALIDATION.Iep_local iep ON iep.IepRefID = prlevel.IepRefID JOIN x_DATAVALIDATION.Student_local st on st.StudentrefID = iep.StudentrefID WHERE 1 = 1 '

SET @query  = ' AND (x_DATAVALIDATION.udf_IsInteger(prlevel.'+@columnname+') = 0 AND prlevel.'+@columnname+' IS NOT NULL)'
SET @vsql = @vsql + @query
EXEC sp_executesql @stmt=@vsql
--PRINT @vsql

SET @sumsql = 'INSERT x_DATAVALIDATION.ValidationSummaryReport (TableName,ErrorMessage,NumberOfRecords)
SELECT ''PresentLevel'',''Expected integer value in '+@columnname+'.'', COUNT(*)
FROM x_DATAVALIDATION.PresentLevel_Local WHERE 1 = 1 '
