...
IF EXISTS (SELECT 1 FROM sys.schemas s join sys.objects o on s.schema_id = o.schema_id where s.name = 'x_DATAVALIDATION' and o.name = 'Check_PostSchoolConsider_Specifications')
DROP PROC x_DATAVALIDATION.Check_PostSchoolConsider_Specifications
GO
/*
To check the specfication of Goal file with our data specification
*/
CREATE PROC x_DATAVALIDATION.Check_PostSchoolConsider_Specifications
AS
BEGIN

DECLARE @sql nVARCHAR(MAX)

SET @sql = 'DELETE x_DATAVALIDATION.PostSchoolConsider'
EXEC sp_executesql @stmt = @sql

---Validated data
DECLARE @sqlvalidated VARCHAR(MAX)
SET @sqlvalidated = 
'INSERT  x_DATAVALIDATION.PostSchoolConsider  
SELECT g.Line_No '
...
FROM x_DATAVALIDATION.ValidationRules WHERE TableName = 'PostSchoolConsider' 
...
WHILE @Count<=@MaxCount
    ...
    END
--print @sel
SET @sqlvalidated = @sqlvalidated + @sel+ ' FROM x_DATAVALIDATION.PostSchoolConsider_Local g'
...
FROM x_DATAVALIDATION.ValidationRules WHERE TableName = 'PostSchoolConsider' AND IsRequired =1
...
FROM x_DATAVALIDATION.ValidationRules WHERE TableName = 'PostSchoolConsider' and IsFlagfield = 1
...
FROM x_DATAVALIDATION.ValidationRules WHERE TableName = 'PostSchoolConsider' AND IsFkRelation = 1
SET @Count = 1
SET @Txtfkrel = ''
...
FROM x_DATAVALIDATION.ValidationRules WHERE TableName = 'PostSchoolConsider' AND IsLookupColumn = 1
SET @Count = 1
SET @Txtlookup = ''
...
SELECT 'PostSchoolConsider','SuccessfulRecords',COUNT(*)
FROM x_DATAVALIDATION.PostSchoolConsider
...
SELECT ''PostSchoolConsider'',''The field '+@columnname+' is required but not found.'',pssclc.Line_No
,ISNULL(CONVERT(VARCHAR(max),st.studentLocalID),'''')+''|''+ISNULL(CONVERT(VARCHAR(max),st.FirstName),'''')+''|''+ISNULL(CONVERT(VARCHAR(max),st.LastName),'''')+''|''+ISNULL(CONVERT(VARCHAR(max),pssclc.IepRefID),'''')+''|''+ISNULL(CONVERT(VARCHAR(max),pssclc.PostSchoolRefID),'''')+''|''+ISNULL(CONVERT(VARCHAR(max),pssclc.ProjectedGradDate),'''') +''|''+ISNULL(CONVERT(VARCHAR(max),pssclc.CompletionDocType),'''') +''|''+ISNULL(CONVERT(VARCHAR(max),pssclc.AgeMajority),'''') +''|''+ISNULL(CONVERT(VARCHAR(max),pssclc.TransferRights),'''')  
FROM x_DATAVALIDATION.PostSchoolConsider_Local pssclc JOIN x_DATAVALIDATION.Iep_local iep ON iep.IepRefID = pssclc.IepRefID 
JOIN x_DATAVALIDATION.Student_local st on st.StudentrefID = iep.StudentrefID WHERE 1 = 1'

SET @query  = ' AND (pssclc.'+@columnname+' IS NULL)'
SET @vsql = @vsql + @query
--PRINT @vsql
...
SELECT ''PostSchoolConsider'',''The field '+@columnname+' is required but not found.'', COUNT(*)
FROM x_DATAVALIDATION.PostSchoolConsider_Local WHERE 1 = 1 '
...
SET @vsql = 'INSERT x_DATAVALIDATION.ValidationReport (TableName,ErrorMessage,LineNumber,Line)
SELECT ''PostSchoolConsider'','''+@columnname+' field will be truncated at '+@datalength+' characters.'',pssclc.Line_No
,ISNULL(CONVERT(VARCHAR(max),st.studentLocalID),'''')+''|''+ISNULL(CONVERT(VARCHAR(max),st.FirstName),'''')+''|''+ISNULL(CONVERT(VARCHAR(max),st.LastName),'''')+''|''+ISNULL(CONVERT(VARCHAR(max),pssclc.IepRefID),'''')+''|''+ISNULL(CONVERT(VARCHAR(max),pssclc.PostSchoolRefID),'''')+''|''+ISNULL(CONVERT(VARCHAR(max),pssclc.ProjectedGradDate),'''') +''|''+ISNULL(CONVERT(VARCHAR(max),pssclc.CompletionDocType),'''') +''|''+ISNULL(CONVERT(VARCHAR(max),pssclc.AgeMajority),'''') +''|''+ISNULL(CONVERT(VARCHAR(max),pssclc.TransferRights),'''')  
FROM x_DATAVALIDATION.PostSchoolConsider_Local pssclc JOIN x_DATAVALIDATION.Iep_local iep ON iep.IepRefID = pssclc.IepRefID 
JOIN x_DATAVALIDATION.Student_local st on st.StudentrefID = iep.StudentrefID WHERE 1 = 1'

SET @query  = ' AND ((DATALENGTH (REPLACE(pssclc.'+@columnname+','''''''',''''))/2) > '+@datalength+' AND pssclc.'+@columnname+' IS NOT NULL)'
SET @vsql = @vsql + @query
EXEC sp_executesql @stmt=@vsql
--PRINT @vsql

SET @sumsql = 'INSERT x_DATAVALIDATION.ValidationSummaryReport (TableName,ErrorMessage,NumberOfRecords)
SELECT ''PostSchoolConsider'','''+@columnname+' field will be truncated at '+@datalength+' characters.'', COUNT(*)
FROM x_DATAVALIDATION.PostSchoolConsider_Local WHERE 1 = 1 '
...
SET @vsql = 'INSERT x_DATAVALIDATION.ValidationReport (TableName,ErrorMessage,LineNumber,Line)
SELECT ''PostSchoolConsider'',''No '+@parenttable+' record found for '+@tablename+'.'',pssclc.Line_No,ISNULL(CONVERT(VARCHAR(max),st.studentLocalID),'''')+''|''+ISNULL(CONVERT(VARCHAR(max),st.FirstName),'''')+''|''+ISNULL(CONVERT(VARCHAR(max),st.LastName),'''')+''|''+ISNULL(CONVERT(VARCHAR(max),pssclc.GoalRefID),'''')+''|''+ISNULL(CONVERT(VARCHAR(max),pssclc.IepRefID),'''')+''|''+ISNULL(CONVERT(VARCHAR(max),pssclc.Sequence),'''')+''|''+ISNULL(CONVERT(VARCHAR(max),pssclc.GoalAreaCode),'''')+''|''+ISNULL(CONVERT(VARCHAR(max),pssclc.SCInstructional),'''')+''|''+ISNULL(CONVERT(VARCHAR(max),pssclc.SCTransition),'''')+''|''+ISNULL(CONVERT(VARCHAR(max),pssclc.SCRelatedService),'''')+''|''+ISNULL(CONVERT(VARCHAR(max),pssclc.IsEsy),'''')+''|''+ISNULL(CONVERT(VARCHAR(max),pssclc.UNITOFMEASUREMENT),'''')+''|''+ISNULL(CONVERT(VARCHAR(max),pssclc.BASELINEDATAPOINT),'''')+''|''+ISNULL(CONVERT(VARCHAR(max),pssclc.EVALUATIONMETHOD),'''')+''|''+ISNULL(CONVERT(VARCHAR(max),pssclc.GoalStatement),'''')
FROM x_DATAVALIDATION.PostSchoolConsider_Local goal JOIN x_DATAVALIDATION.Iep_local iep ON iep.IepRefID = pssclc.IepRefID JOIN x_DATAVALIDATION.Student_local st on st.StudentrefID = iep.StudentrefID '

SET @query  = ' LEFT JOIN x_DATAVALIDATION.'+@parenttable+' dt ON pssclc.'+@columnname+' = dt.'+@parentcolumn+' WHERE dt.'+@parentcolumn+' IS NULL'
SET @vsql = @vsql + @query
EXEC sp_executesql @stmt=@vsql
--PRINT @vsql

SET @sumsql = 'INSERT x_DATAVALIDATION.ValidationSummaryReport (TableName,ErrorMessage,NumberOfRecords)
SELECT ''PostSchoolConsider'',''No '+@parenttable+' record found for '+@tablename+'.'', COUNT(*)
FROM x_DATAVALIDATION.PostSchoolConsider_Local goal '
...
SET @vsql = 'INSERT x_DATAVALIDATION.ValidationReport (TableName,ErrorMessage,LineNumber,Line)
SELECT ''PostSchoolConsider'',''Invalid value in '+@columnname+' it must be Y or N.'',pssclc.Line_No
,ISNULL(CONVERT(VARCHAR(max),st.studentLocalID),'''')+''|''+ISNULL(CONVERT(VARCHAR(max),st.FirstName),'''')+''|''+ISNULL(CONVERT(VARCHAR(max),st.LastName),'''')+''|''+ISNULL(CONVERT(VARCHAR(max),pssclc.IepRefID),'''')+''|''+ISNULL(CONVERT(VARCHAR(max),pssclc.PostSchoolRefID),'''')+''|''+ISNULL(CONVERT(VARCHAR(max),pssclc.ProjectedGradDate),'''') +''|''+ISNULL(CONVERT(VARCHAR(max),pssclc.CompletionDocType),'''') +''|''+ISNULL(CONVERT(VARCHAR(max),pssclc.AgeMajority),'''') +''|''+ISNULL(CONVERT(VARCHAR(max),pssclc.TransferRights),'''')  
FROM x_DATAVALIDATION.PostSchoolConsider_Local pssclc JOIN x_DATAVALIDATION.Iep_local iep ON iep.IepRefID = pssclc.IepRefID 
JOIN x_DATAVALIDATION.Student_local st on st.StudentrefID = iep.StudentrefID  '

SET @query  = '  WHERE (pssclc.'+@columnname+' NOT IN  ('+@flagrecords+') AND pssclc.'+@columnname+' IS NOT NULL)'
SET @vsql = @vsql + @query
EXEC sp_executesql @stmt=@vsql
--PRINT @vsql
...
SET @sumsql = 'INSERT x_DATAVALIDATION.ValidationSummaryReport (TableName,ErrorMessage,NumberOfRecords)
SELECT ''PostSchoolConsider'',''Invalid value in '+@columnname+' it must be Y or N.'', COUNT(*)
FROM x_DATAVALIDATION.PostSchoolConsider_Local pssclc '
...
SET @vsql = 'INSERT x_DATAVALIDATION.ValidationReport (TableName,ErrorMessage,LineNumber,Line)
SELECT ''PostSchoolConsider'','''+@columnname+' is duplicated.'',pssclc.Line_No
,ISNULL(CONVERT(VARCHAR(max),st.studentLocalID),'''')+''|''+ISNULL(CONVERT(VARCHAR(max),st.FirstName),'''')+''|''+ISNULL(CONVERT(VARCHAR(max),st.LastName),'''')+''|''+ISNULL(CONVERT(VARCHAR(max),pssclc.IepRefID),'''')+''|''+ISNULL(CONVERT(VARCHAR(max),pssclc.PostSchoolRefID),'''')+''|''+ISNULL(CONVERT(VARCHAR(max),pssclc.ProjectedGradDate),'''') +''|''+ISNULL(CONVERT(VARCHAR(max),pssclc.CompletionDocType),'''') +''|''+ISNULL(CONVERT(VARCHAR(max),pssclc.AgeMajority),'''') +''|''+ISNULL(CONVERT(VARCHAR(max),pssclc.TransferRights),'''')  
FROM x_DATAVALIDATION.PostSchoolConsider_Local pssclc JOIN x_DATAVALIDATION.Iep_local iep ON iep.IepRefID = pssclc.IepRefID 
JOIN x_DATAVALIDATION.Student_local st on st.StudentrefID = iep.StudentrefID '

SET @query  = ' JOIN (SELECT '+@columnname+' FROM x_DATAVALIDATION.PostSchoolConsider_Local GROUP BY '+@columnname+' HAVING COUNT(*)>1) ucpssclc ON ucpssclc.'+@columnname+' = pssclc.'+@columnname+' '
SET @vsql = @vsql + @query
EXEC sp_executesql @stmt=@vsql
--PRINT @vsql

SET @sumsql = 'INSERT x_DATAVALIDATION.ValidationSummaryReport (TableName,ErrorMessage,NumberOfRecords)
SELECT ''PostSchoolConsider'','''+@columnname+' is duplicated.'', COUNT(*)
FROM x_DATAVALIDATION.PostSchoolConsider_Local pssclc JOIN  '

SET @query  = ' (SELECT '+@columnname+' FROM x_DATAVALIDATION.PostSchoolConsider_Local GROUP BY '+@columnname+' HAVING COUNT(*)>1) ucpssclc ON ucpssclc.'+@columnname+' = pssclc.'+@columnname+' '
SET @sumsql = @sumsql + @query
--PRINT @sumsql
EXEC sp_executesql @stmt=@sumsql
...
SET @vsql = 'INSERT x_DATAVALIDATION.ValidationReport (TableName,ErrorMessage,LineNumber,Line)
SELECT ''PostSchoolConsider'',''No lookup value found for '+@columnname+'.  Contact ExcentDataTeam@excent.com .'',pssclc.Line_No
,ISNULL(CONVERT(VARCHAR(max),st.studentLocalID),'''')+''|''+ISNULL(CONVERT(VARCHAR(max),st.FirstName),'''')+''|''+ISNULL(CONVERT(VARCHAR(max),st.LastName),'''')+''|''+ISNULL(CONVERT(VARCHAR(max),pssclc.IepRefID),'''')+''|''+ISNULL(CONVERT(VARCHAR(max),pssclc.PostSchoolRefID),'''')+''|''+ISNULL(CONVERT(VARCHAR(max),pssclc.ProjectedGradDate),'''') +''|''+ISNULL(CONVERT(VARCHAR(max),pssclc.CompletionDocType),'''') +''|''+ISNULL(CONVERT(VARCHAR(max),pssclc.AgeMajority),'''') +''|''+ISNULL(CONVERT(VARCHAR(max),pssclc.TransferRights),'''')  
FROM x_DATAVALIDATION.PostSchoolConsider_Local pssclc JOIN x_DATAVALIDATION.Iep_local iep ON iep.IepRefID = pssclc.IepRefID 
JOIN x_DATAVALIDATION.Student_local st on st.StudentrefID = iep.StudentrefID '

SET @query  = ' WHERE (pssclc.'+@columnname+' NOT IN ( SELECT '+@lookupcolumn+' FROM x_DATAVALIDATION.'+@lookuptable+' WHERE Type = '''+@lookuptype+''') AND pssclc.'+@columnname+' IS NOT NULL)'
SET @vsql = @vsql + @query
EXEC sp_executesql @stmt=@vsql
--PRINT @vsql

SET @sumsql = 'INSERT x_DATAVALIDATION.ValidationSummaryReport (TableName,ErrorMessage,NumberOfRecords)
SELECT ''PostSchoolConsider'',''No lookup value found for '+@columnname+'.'', COUNT(*)
FROM x_DATAVALIDATION.PostSchoolConsider_Local pssclc  '
...
SET @query  = ' WHERE (pssclc.'+@columnname+' NOT IN ( SELECT '+@lookupcolumn+' FROM x_DATAVALIDATION.'+@lookuptable+') AND pssclc.'+@columnname+' IS NOT NULL)'
SET @vsql = @vsql + @query
EXEC sp_executesql @stmt=@vsql
--PRINT @vsql
...
SET @vsql = 'INSERT x_DATAVALIDATION.ValidationReport (TableName,ErrorMessage,LineNumber,Line)
SELECT ''PostSchoolConsider'',''Expected date value in '+@columnname+'.'',pssclc.Line_No
,ISNULL(CONVERT(VARCHAR(max),st.studentLocalID),'''')+''|''+ISNULL(CONVERT(VARCHAR(max),st.FirstName),'''')+''|''+ISNULL(CONVERT(VARCHAR(max),st.LastName),'''')+''|''+ISNULL(CONVERT(VARCHAR(max),pssclc.IepRefID),'''')+''|''+ISNULL(CONVERT(VARCHAR(max),pssclc.PostSchoolRefID),'''')+''|''+ISNULL(CONVERT(VARCHAR(max),pssclc.ProjectedGradDate),'''') +''|''+ISNULL(CONVERT(VARCHAR(max),pssclc.CompletionDocType),'''') +''|''+ISNULL(CONVERT(VARCHAR(max),pssclc.AgeMajority),'''') +''|''+ISNULL(CONVERT(VARCHAR(max),pssclc.TransferRights),'''')  
FROM x_DATAVALIDATION.PostSchoolConsider_Local pssclc JOIN x_DATAVALIDATION.Iep_local iep ON iep.IepRefID = pssclc.IepRefID 
JOIN x_DATAVALIDATION.Student_local st on st.StudentrefID = iep.StudentrefID WHERE 1 = 1 '

SET @query  = ' AND (ISDATE(''pssclc.'+@columnname+''') = 0 AND pssclc.'+@columnname+' IS NOT NULL)'
SET @vsql = @vsql + @query
EXEC sp_executesql @stmt=@vsql
--PRINT @vsql

SET @sumsql = 'INSERT x_DATAVALIDATION.ValidationSummaryReport (TableName,ErrorMessage,NumberOfRecords)
SELECT ''PostSchoolConsider'',''Expected date value in '+@columnname+'.'', COUNT(*)
FROM x_DATAVALIDATION.PostSchoolConsider_Local WHERE 1 = 1 '
...
SET @vsql = 'INSERT x_DATAVALIDATION.ValidationReport (TableName,ErrorMessage,LineNumber,Line)
SELECT ''PostSchoolConsider'',''Expected integer value in '+@columnname+'.'',pssclc.Line_No
,ISNULL(CONVERT(VARCHAR(max),st.studentLocalID),'''')+''|''+ISNULL(CONVERT(VARCHAR(max),st.FirstName),'''')+''|''+ISNULL(CONVERT(VARCHAR(max),st.LastName),'''')+''|''+ISNULL(CONVERT(VARCHAR(max),pssclc.IepRefID),'''')+''|''+ISNULL(CONVERT(VARCHAR(max),pssclc.PostSchoolRefID),'''')+''|''+ISNULL(CONVERT(VARCHAR(max),pssclc.ProjectedGradDate),'''') +''|''+ISNULL(CONVERT(VARCHAR(max),pssclc.CompletionDocType),'''') +''|''+ISNULL(CONVERT(VARCHAR(max),pssclc.AgeMajority),'''') +''|''+ISNULL(CONVERT(VARCHAR(max),pssclc.TransferRights),'''')  
FROM x_DATAVALIDATION.PostSchoolConsider_Local pssclc JOIN x_DATAVALIDATION.Iep_local iep ON iep.IepRefID = pssclc.IepRefID 
JOIN x_DATAVALIDATION.Student_local st on st.StudentrefID = iep.StudentrefID WHERE 1 = 1 '

SET @query  = ' AND (x_DATAVALIDATION.udf_IsInteger(pssclc.'+@columnname+') = 0 AND pssclc.'+@columnname+' IS NOT NULL)'
SET @vsql = @vsql + @query
EXEC sp_executesql @stmt=@vsql
--PRINT @vsql

SET @sumsql = 'INSERT x_DATAVALIDATION.ValidationSummaryReport (TableName,ErrorMessage,NumberOfRecords)
SELECT ''PostSchoolConsider'',''Expected integer value in '+@columnname+'.'', COUNT(*)
FROM x_DATAVALIDATION.PostSchoolConsider_Local WHERE 1 = 1 '
