...
IF EXISTS (SELECT 1 FROM sys.schemas s join sys.objects o on s.schema_id = o.schema_id where s.name = 'x_DATAVALIDATION' and o.name = 'Check_Evaluation_Specifications')
DROP PROC x_DATAVALIDATION.Check_Evaluation_Specifications
GO
/*
To check the specfication of Goal file with our data specification
*/
CREATE PROC x_DATAVALIDATION.Check_Evaluation_Specifications
AS
BEGIN

DECLARE @sql nVARCHAR(MAX)

SET @sql = 'DELETE x_DATAVALIDATION.Evaluation'
EXEC sp_executesql @stmt = @sql

---Validated data
DECLARE @sqlvalidated VARCHAR(MAX)
SET @sqlvalidated = 
'INSERT  x_DATAVALIDATION.Evaluation  
SELECT g.Line_No '
...
FROM x_DATAVALIDATION.ValidationRules WHERE TableName = 'Evaluation' 
...
WHILE @Count<=@MaxCount
    ...
    END
--print @sel
SET @sqlvalidated = @sqlvalidated + @sel+ ' FROM x_DATAVALIDATION.Evaluation_Local g'
...
FROM x_DATAVALIDATION.ValidationRules WHERE TableName = 'Evaluation' AND IsRequired =1
...
FROM x_DATAVALIDATION.ValidationRules WHERE TableName = 'Evaluation' and IsFlagfield = 1
...
FROM x_DATAVALIDATION.ValidationRules WHERE TableName = 'Evaluation' AND IsFkRelation = 1
SET @Count = 1
SET @Txtfkrel = ''
...
FROM x_DATAVALIDATION.ValidationRules WHERE TableName = 'Evaluation' AND IsLookupColumn = 1
SET @Count = 1
SET @Txtlookup = ''
...
SELECT 'Evaluation','SuccessfulRecords',COUNT(*)
FROM x_DATAVALIDATION.Evaluation
...
SELECT ''Evaluation'',''The field '+@columnname+' is required but not found.'',evl.Line_No,ISNULL(CONVERT(VARCHAR(max),st.studentLocalID),'''')+''|''+ISNULL(CONVERT(VARCHAR(max),st.FirstName),'''')+''|''+ISNULL(CONVERT(VARCHAR(max),st.LastName),'''')+''|''+ISNULL(CONVERT(VARCHAR(max),evl.IepRefID),'''')+''|''+ISNULL(CONVERT(VARCHAR(max),evl.EvaluationRefID),'''')+''|''+ISNULL(CONVERT(VARCHAR(max),evl.VisionScreen),'''')+''|''+ISNULL(CONVERT(VARCHAR(max),evl.HearingScreen),'''')+''|''+ISNULL(CONVERT(VARCHAR(max),evl.EvaluationSummary),'''') FROM x_DATAVALIDATION.Evaluation_Local evl JOIN x_DATAVALIDATION.Iep_local iep ON iep.IepRefID = evl.IepRefID JOIN x_DATAVALIDATION.Student_local st on st.StudentrefID = iep.StudentrefID WHERE 1 = 1'

SET @query  = ' AND (evl.'+@columnname+' IS NULL)'
SET @vsql = @vsql + @query
--PRINT @vsql
...
SELECT ''Evaluation'',''The field '+@columnname+' is required but not found.'', COUNT(*)
FROM x_DATAVALIDATION.Evaluation_Local WHERE 1 = 1 '
...
SET @vsql = 'INSERT x_DATAVALIDATION.ValidationReport (TableName,ErrorMessage,LineNumber,Line)
SELECT ''Evaluation'','''+@columnname+' field will be truncated at '+@datalength+' characters.'',evl.Line_No,ISNULL(CONVERT(VARCHAR(max),st.studentLocalID),'''')+''|''+ISNULL(CONVERT(VARCHAR(max),st.FirstName),'''')+''|''+ISNULL(CONVERT(VARCHAR(max),st.LastName),'''')+''|''+ISNULL(CONVERT(VARCHAR(max),evl.IepRefID),'''')+''|''+ISNULL(CONVERT(VARCHAR(max),evl.EvaluationRefID),'''')+''|''+ISNULL(CONVERT(VARCHAR(max),evl.VisionScreen),'''')+''|''+ISNULL(CONVERT(VARCHAR(max),evl.HearingScreen),'''')+''|''+ISNULL(CONVERT(VARCHAR(max),evl.EvaluationSummary),'''') FROM x_DATAVALIDATION.Evaluation_Local evl JOIN x_DATAVALIDATION.Iep_local iep ON iep.IepRefID = evl.IepRefID JOIN x_DATAVALIDATION.Student_local st on st.StudentrefID = iep.StudentrefID WHERE 1 = 1'

SET @query  = ' AND ((DATALENGTH (REPLACE(evl.'+@columnname+','''''''',''''))/2) > '+@datalength+' AND evl.'+@columnname+' IS NOT NULL)'
SET @vsql = @vsql + @query
EXEC sp_executesql @stmt=@vsql
--PRINT @vsql

SET @sumsql = 'INSERT x_DATAVALIDATION.ValidationSummaryReport (TableName,ErrorMessage,NumberOfRecords)
SELECT ''Evaluation'','''+@columnname+' field will be truncated at '+@datalength+' characters.'', COUNT(*)
FROM x_DATAVALIDATION.Evaluation_Local WHERE 1 = 1 '
...
SET @vsql = 'INSERT x_DATAVALIDATION.ValidationReport (TableName,ErrorMessage,LineNumber,Line)
SELECT ''Evaluation'',''No '+@parenttable+' record found for '+@tablename+'.'',evl.Line_No,ISNULL(CONVERT(VARCHAR(max),st.studentLocalID),'''')+''|''+ISNULL(CONVERT(VARCHAR(max),st.FirstName),'''')+''|''+ISNULL(CONVERT(VARCHAR(max),st.LastName),'''')+''|''+ISNULL(CONVERT(VARCHAR(max),evl.IepRefID),'''')+''|''+ISNULL(CONVERT(VARCHAR(max),evl.EvaluationRefID),'''')+''|''+ISNULL(CONVERT(VARCHAR(max),evl.VisionScreen),'''')+''|''+ISNULL(CONVERT(VARCHAR(max),evl.HearingScreen),'''')+''|''+ISNULL(CONVERT(VARCHAR(max),evl.EvaluationSummary),'''') FROM x_DATAVALIDATION.Evaluation_Local evl JOIN x_DATAVALIDATION.Iep_local iep ON iep.IepRefID = evl.IepRefID JOIN x_DATAVALIDATION.Student_local st on st.StudentrefID = iep.StudentrefID '

SET @query  = ' LEFT JOIN x_DATAVALIDATION.'+@parenttable+' dt ON evl.'+@columnname+' = dt.'+@parentcolumn+' WHERE dt.'+@parentcolumn+' IS NULL'
SET @vsql = @vsql + @query
EXEC sp_executesql @stmt=@vsql
--PRINT @vsql

SET @sumsql = 'INSERT x_DATAVALIDATION.ValidationSummaryReport (TableName,ErrorMessage,NumberOfRecords)
SELECT ''Evaluation'',''No '+@parenttable+' record found for '+@tablename+'.'', COUNT(*)
FROM x_DATAVALIDATION.Evaluation_Local goal '
...
SET @vsql = 'INSERT x_DATAVALIDATION.ValidationReport (TableName,ErrorMessage,LineNumber,Line)
SELECT ''Evaluation'',''Invalid value in '+@columnname+' it must be Y or N.'',evl.Line_No,ISNULL(CONVERT(VARCHAR(max),st.studentLocalID),'''')+''|''+ISNULL(CONVERT(VARCHAR(max),st.FirstName),'''')+''|''+ISNULL(CONVERT(VARCHAR(max),st.LastName),'''')+''|''+ISNULL(CONVERT(VARCHAR(max),evl.IepRefID),'''')+''|''+ISNULL(CONVERT(VARCHAR(max),evl.EvaluationRefID),'''')+''|''+ISNULL(CONVERT(VARCHAR(max),evl.VisionScreen),'''')+''|''+ISNULL(CONVERT(VARCHAR(max),evl.HearingScreen),'''')+''|''+ISNULL(CONVERT(VARCHAR(max),evl.EvaluationSummary),'''') FROM x_DATAVALIDATION.Evaluation_Local evl JOIN x_DATAVALIDATION.Iep_local iep ON iep.IepRefID = evl.IepRefID JOIN x_DATAVALIDATION.Student_local st on st.StudentrefID = iep.StudentrefID'

SET @query  = '  WHERE (evl.'+@columnname+' NOT IN  ('+@flagrecords+') AND evl.'+@columnname+' IS NOT NULL)'
SET @vsql = @vsql + @query
EXEC sp_executesql @stmt=@vsql
--PRINT @vsql
...
SET @sumsql = 'INSERT x_DATAVALIDATION.ValidationSummaryReport (TableName,ErrorMessage,NumberOfRecords)
SELECT ''Evaluation'',''Invalid value in '+@columnname+' it must be Y or N.'', COUNT(*)
FROM x_DATAVALIDATION.Evaluation_Local evl '
...
SET @vsql = 'INSERT x_DATAVALIDATION.ValidationReport (TableName,ErrorMessage,LineNumber,Line)
SELECT ''Evaluation'','''+@columnname+' is duplicated.'',evl.Line_No,ISNULL(CONVERT(VARCHAR(max),st.studentLocalID),'''')+''|''+ISNULL(CONVERT(VARCHAR(max),st.FirstName),'''')+''|''+ISNULL(CONVERT(VARCHAR(max),st.LastName),'''')+''|''+ISNULL(CONVERT(VARCHAR(max),evl.IepRefID),'''')+''|''+ISNULL(CONVERT(VARCHAR(max),evl.EvaluationRefID),'''')+''|''+ISNULL(CONVERT(VARCHAR(max),evl.VisionScreen),'''')+''|''+ISNULL(CONVERT(VARCHAR(max),evl.HearingScreen),'''')+''|''+ISNULL(CONVERT(VARCHAR(max),evl.EvaluationSummary),'''') FROM x_DATAVALIDATION.Evaluation_Local evl JOIN x_DATAVALIDATION.Iep_local iep ON iep.IepRefID = evl.IepRefID JOIN x_DATAVALIDATION.Student_local st on st.StudentrefID = iep.StudentrefID'

SET @query  = ' JOIN (SELECT '+@columnname+' FROM x_DATAVALIDATION.Evaluation_Local GROUP BY '+@columnname+' HAVING COUNT(*)>1) ucevl ON ucevl.'+@columnname+' = evl.'+@columnname+' '
SET @vsql = @vsql + @query
EXEC sp_executesql @stmt=@vsql
--PRINT @vsql

SET @sumsql = 'INSERT x_DATAVALIDATION.ValidationSummaryReport (TableName,ErrorMessage,NumberOfRecords)
SELECT ''Evaluation'','''+@columnname+' is duplicated.'', COUNT(*)
FROM x_DATAVALIDATION.Evaluation_Local evl JOIN  '

SET @query  = ' (SELECT '+@columnname+' FROM x_DATAVALIDATION.Evaluation_Local GROUP BY '+@columnname+' HAVING COUNT(*)>1) ucevl ON ucevl.'+@columnname+' = evl.'+@columnname+' '
SET @sumsql = @sumsql + @query
--PRINT @sumsql
EXEC sp_executesql @stmt=@sumsql
...
SET @vsql = 'INSERT x_DATAVALIDATION.ValidationReport (TableName,ErrorMessage,LineNumber,Line)
SELECT ''Evaluation'',''No lookup value found for '+@columnname+'.  Contact ExcentDataTeam@excent.com .'',evl.Line_No,ISNULL(CONVERT(VARCHAR(max),st.studentLocalID),'''')+''|''+ISNULL(CONVERT(VARCHAR(max),st.FirstName),'''')+''|''+ISNULL(CONVERT(VARCHAR(max),st.LastName),'''')+''|''+ISNULL(CONVERT(VARCHAR(max),evl.IepRefID),'''')+''|''+ISNULL(CONVERT(VARCHAR(max),evl.EvaluationRefID),'''')+''|''+ISNULL(CONVERT(VARCHAR(max),evl.VisionScreen),'''')+''|''+ISNULL(CONVERT(VARCHAR(max),evl.HearingScreen),'''')+''|''+ISNULL(CONVERT(VARCHAR(max),evl.EvaluationSummary),'''') FROM x_DATAVALIDATION.Evaluation_Local evl JOIN x_DATAVALIDATION.Iep_local iep ON iep.IepRefID = evl.IepRefID JOIN x_DATAVALIDATION.Student_local st on st.StudentrefID = iep.StudentrefID '

SET @query  = ' WHERE (evl.'+@columnname+' NOT IN ( SELECT '+@lookupcolumn+' FROM x_DATAVALIDATION.'+@lookuptable+' WHERE Type = '''+@lookuptype+''') AND evl.'+@columnname+' IS NOT NULL)'
SET @vsql = @vsql + @query
EXEC sp_executesql @stmt=@vsql
--PRINT @vsql

SET @sumsql = 'INSERT x_DATAVALIDATION.ValidationSummaryReport (TableName,ErrorMessage,NumberOfRecords)
SELECT ''Evaluation'',''No lookup value found for '+@columnname+'.'', COUNT(*)
FROM x_DATAVALIDATION.Evaluation_Local evl  '
...
SET @query  = ' WHERE (evl.'+@columnname+' NOT IN ( SELECT '+@lookupcolumn+' FROM x_DATAVALIDATION.'+@lookuptable+') AND evl.'+@columnname+' IS NOT NULL)'
SET @vsql = @vsql + @query
EXEC sp_executesql @stmt=@vsql
--PRINT @vsql
...
SET @vsql = 'INSERT x_DATAVALIDATION.ValidationReport (TableName,ErrorMessage,LineNumber,Line)
SELECT ''Evaluation'',''Expected date value in '+@columnname+'.'',evl.Line_No,ISNULL(CONVERT(VARCHAR(max),st.studentLocalID),'''')+''|''+ISNULL(CONVERT(VARCHAR(max),st.FirstName),'''')+''|''+ISNULL(CONVERT(VARCHAR(max),st.LastName),'''')+''|''+ISNULL(CONVERT(VARCHAR(max),evl.IepRefID),'''')+''|''+ISNULL(CONVERT(VARCHAR(max),evl.EvaluationRefID),'''')+''|''+ISNULL(CONVERT(VARCHAR(max),evl.VisionScreen),'''')+''|''+ISNULL(CONVERT(VARCHAR(max),evl.HearingScreen),'''')+''|''+ISNULL(CONVERT(VARCHAR(max),evl.EvaluationSummary),'''') FROM x_DATAVALIDATION.Evaluation_Local evl JOIN x_DATAVALIDATION.Iep_local iep ON iep.IepRefID = evl.IepRefID JOIN x_DATAVALIDATION.Student_local st on st.StudentrefID = iep.StudentrefID WHERE 1 = 1 '

SET @query  = ' AND (ISDATE(''evl.'+@columnname+''') = 0 AND evl.'+@columnname+' IS NOT NULL)'
SET @vsql = @vsql + @query
EXEC sp_executesql @stmt=@vsql
--PRINT @vsql

SET @sumsql = 'INSERT x_DATAVALIDATION.ValidationSummaryReport (TableName,ErrorMessage,NumberOfRecords)
SELECT ''Evaluation'',''Expected date value in '+@columnname+'.'', COUNT(*)
FROM x_DATAVALIDATION.Evaluation_Local WHERE 1 = 1 '
...
SET @vsql = 'INSERT x_DATAVALIDATION.ValidationReport (TableName,ErrorMessage,LineNumber,Line)
SELECT ''Evaluation'',''Expected integer value in '+@columnname+'.'',evl.Line_No,ISNULL(CONVERT(VARCHAR(max),st.studentLocalID),'''')+''|''+ISNULL(CONVERT(VARCHAR(max),st.FirstName),'''')+''|''+ISNULL(CONVERT(VARCHAR(max),st.LastName),'''')+''|''+ISNULL(CONVERT(VARCHAR(max),evl.IepRefID),'''')+''|''+ISNULL(CONVERT(VARCHAR(max),evl.EvaluationRefID),'''')+''|''+ISNULL(CONVERT(VARCHAR(max),evl.VisionScreen),'''')+''|''+ISNULL(CONVERT(VARCHAR(max),evl.HearingScreen),'''')+''|''+ISNULL(CONVERT(VARCHAR(max),evl.EvaluationSummary),'''') FROM x_DATAVALIDATION.Evaluation_Local evl JOIN x_DATAVALIDATION.Iep_local iep ON iep.IepRefID = evl.IepRefID JOIN x_DATAVALIDATION.Student_local st on st.StudentrefID = iep.StudentrefID WHERE 1 = 1 '

SET @query  = ' AND (x_DATAVALIDATION.udf_IsInteger(evl.'+@columnname+') = 0 AND evl.'+@columnname+' IS NOT NULL)'
SET @vsql = @vsql + @query
EXEC sp_executesql @stmt=@vsql
--PRINT @vsql

SET @sumsql = 'INSERT x_DATAVALIDATION.ValidationSummaryReport (TableName,ErrorMessage,NumberOfRecords)
SELECT ''Evaluation'',''Expected integer value in '+@columnname+'.'', COUNT(*)
FROM x_DATAVALIDATION.Evaluation_Local WHERE 1 = 1 '
