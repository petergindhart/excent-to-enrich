...
IF  EXISTS (SELECT * FROM dbo.sysobjects WHERE id = OBJECT_ID(N'CodeDescLook_EO') AND OBJECTPROPERTY(id, N'IsView') = 1)
DROP VIEW CodeDescLook_EO
GO

CREATE VIEW CodeDescLook_EO
AS
select 
...
IF  EXISTS (SELECT 1 FROM dbo.sysobjects WHERE id = OBJECT_ID(N'SelectLists_src') AND type in (N'U'))
DROP TABLE dbo.SelectLists_src
GO

SELECT
Line_No = IDENTITY(INT,1,1),  
Type, SubType, EnrichID = NULL, StateCode, LegacySpedCode = Code, EnrichLabel = Label
INTO dbo.SelectLists_src
FROM CodeDescLook_EO
...
SELECT
Line_No ,  
Type, SubType, EnrichID = NULL, StateCode, LegacySpedCode , EnrichLabel
FROM dbo.SelectLists_src

--select * from dbo.SelectLists_EO
--select * from SelectLists_Src