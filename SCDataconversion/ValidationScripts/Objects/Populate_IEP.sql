IF EXISTS (SELECT 1 FROM sys.schemas s join sys.objects o on s.schema_id = o.schema_id where s.name = 'x_DATAVALIDATION' and o.name = 'Populate_IEP')
DROP PROC x_DATAVALIDATION.Populate_IEP
GO

CREATE PROC x_DATAVALIDATION.Populate_IEP
AS
BEGIN
...
SET @dsql = 'DROP TABLE X_DATAVALIDATION.IEP_LOCAL'
EXEC sp_executesql @stmt=@dsql

SET @sql='SELECT * INTO  X_DATAVALIDATION.IEP_LOCAL FROM  '+@Linkedserver+'.'+@DatabaseName+'.'+@DatabaseOwner+'.'+'IEP_EO'
--PRINT @sql
