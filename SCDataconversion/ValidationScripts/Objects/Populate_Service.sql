IF EXISTS (SELECT 1 FROM sys.schemas s join sys.objects o on s.schema_id = o.schema_id where s.name = 'x_DATAVALIDATION' and o.name = 'Populate_Service')
DROP PROC x_DATAVALIDATION.Populate_Service
GO

CREATE PROC x_DATAVALIDATION.Populate_Service
AS
BEGIN
...
SET @dsql = 'DROP TABLE X_DATAVALIDATION.Service_LOCAL'
EXEC sp_executesql @stmt=@dsql

SET @sql='SELECT * INTO  X_DATAVALIDATION.Service_LOCAL FROM  '+@Linkedserver+'.'+@DatabaseName+'.'+@DatabaseOwner+'.'+'Service_EO'
--PRINT @sql
