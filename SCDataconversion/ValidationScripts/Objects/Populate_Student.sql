IF EXISTS (SELECT 1 FROM sys.schemas s join sys.objects o on s.schema_id = o.schema_id where s.name = 'x_DATAVALIDATION' and o.name = 'Populate_Student')
DROP PROC x_DATAVALIDATION.Populate_Student
GO

CREATE PROC x_DATAVALIDATION.Populate_Student
AS
BEGIN
...
SET @dsql = 'DROP TABLE X_DATAVALIDATION.Student_LOCAL'
EXEC sp_executesql @stmt=@dsql

SET @sql='SELECT * INTO  X_DATAVALIDATION.Student_LOCAL FROM  '+@Linkedserver+'.'+@DatabaseName+'.'+@DatabaseOwner+'.'+'Student_EO'
--PRINT @sql
