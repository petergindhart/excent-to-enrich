IF EXISTS (SELECT 1 FROM sys.schemas s join sys.objects o on s.schema_id = o.schema_id where s.name = 'x_DATAVALIDATION' and o.name = 'Populate_Goal')
DROP PROC x_DATAVALIDATION.Populate_Goal
GO

CREATE PROC x_DATAVALIDATION.Populate_Goal
AS
BEGIN
...
SET @dsql = 'DROP TABLE X_DATAVALIDATION.Goal_LOCAL'
EXEC sp_executesql @stmt=@dsql

SET @sql='SELECT * INTO  X_DATAVALIDATION.Goal_LOCAL FROM  '+@Linkedserver+'.'+@DatabaseName+'.'+@DatabaseOwner+'.'+'Goal_EO'
--PRINT @sql
