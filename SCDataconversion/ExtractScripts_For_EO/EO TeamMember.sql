...
AS
SELECT  Line_No=Row_Number() OVER (ORDER BY (SELECT 1)),Sta.Email as StaffEmail,stu.GStudentID as StudentRefID, IsCaseManager =  case mem.CaseMgr when  1 then 'Y' when  0 then 'N' ELSE '' END
