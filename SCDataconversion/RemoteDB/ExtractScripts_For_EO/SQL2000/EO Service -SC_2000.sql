...
IF  EXISTS (SELECT * FROM dbo.sysobjects WHERE id = OBJECT_ID(N'dbo.vw_Service') AND OBJECTPROPERTY(id, N'IsView') = 1)
DROP VIEW dbo.vw_Service
GO
...
service.ServiceType,service.ServiceRefID,service.IepRefID, service.ServiceDefinitionCode,service.BeginDate,service.EndDate,service.IsRelated,service.IsDirect,service.ExcludesFromGenEd,service.ServiceLocationCode,service.ServiceProviderTitleCode,service.Sequence,service.IsESY,service.ServiceTime,service.ServiceFrequencyCode,service.ServiceProviderSSN,service.StaffEmail,service.ServiceAreaText
FROM (
select 	distinct
    ServiceType = v.Type, 
	v.ServiceRefId, 
...
IF  EXISTS (SELECT 1 FROM dbo.sysobjects WHERE id = OBJECT_ID(N'dbo.Service_src') AND type in (N'U'))
DROP TABLE dbo.Service_src
GO
SELECT 
Line_No= IDENTITY(INT,1,1),
ServiceType,ServiceRefID,IepRefID, ServiceDefinitionCode,BeginDate,EndDate,IsRelated,IsDirect,ExcludesFromGenEd,ServiceLocationCode,ServiceProviderTitleCode,Sequence,IsESY,ServiceTime
,ServiceFrequencyCode,ServiceProviderSSN,StaffEmail,ServiceAreaText
INTO dbo.Service_src
FROM vw_Service
...
ServiceType,ServiceRefID,IepRefID, ServiceDefinitionCode,BeginDate,EndDate,IsRelated,IsDirect,ExcludesFromGenEd,ServiceLocationCode,ServiceProviderTitleCode,Sequence,IsESY,ServiceTime
,ServiceFrequencyCode,ServiceProviderSSN,StaffEmail,ServiceAreaText
FROM dbo.Service_src

--select * from dbo.Service_EO