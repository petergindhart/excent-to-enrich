...
set connection=server=.;Integrated Security=True;database=Enrich_DCB5_SC_Greenwood51
...
SQLCMD %remoteConnStr% -i ..\DbScripts\x_VALIDATION\Localization\EO\SC\ExtractScripts\SQL2005\UpgradeRemoteDB.sql -o%locFolder%\log_UpgradeRemoteDB.txt
...
SQLCMD %connStr% -i ..\DbScripts\x_VALIDATION\Objects\Create_FTPdtsConfig.sql -h-1 -o%locFolder%\ValidationReport_UploadFTP_IEPwStuInfo.dtsConfig

SQLCMD %connStr% -i ..\DbScripts\x_VALIDATION\Objects\Create_Upload_FTP_txt.sql -h-1 -o%locFolder%\ValidationReport_Upload_FTP.txt
