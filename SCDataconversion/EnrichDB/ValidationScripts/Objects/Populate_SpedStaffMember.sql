IF EXISTS (SELECT 1 FROM sys.schemas s join sys.objects o on s.schema_id = o.schema_id where s.name = 'x_DATAVALIDATION' and o.name = 'Populate_SpedStaffMember')
DROP PROC x_DATAVALIDATION.Populate_SpedStaffMember
GO

CREATE PROC x_DATAVALIDATION.Populate_SpedStaffMember
AS
BEGIN
...
SET @dsql = 'DROP TABLE X_DATAVALIDATION.SpedStaffMember_LOCAL'
EXEC sp_executesql @stmt=@dsql

SET @sql='SELECT * INTO  X_DATAVALIDATION.SpedStaffMember_LOCAL FROM  '+@Linkedserver+'.'+@DatabaseName+'.'+@DatabaseOwner+'.'+'SpedStaffMember_EO'
--PRINT @sql
